...
const declaredEvents = Object.keys(protocol.NEXT_S2C);
const bindMatches = [...socketSource.matchAll(/bindServerEvent\(NEXT_S2C\.([A-Za-z0-9_]+)\)/g)].map((match) => match[1]);
const exposedMatches = [...socketSource.matchAll(/^\s{2}on([A-Z][A-Za-z0-9]+)\([^\n]*NEXT_S2C\.([A-Za-z0-9_]+)/gm)];
...
if (/socket\.on\(NEXT_S2C\.Kick,/.test(socketSource)) {
...
function main() {
  ...
  if (boundNotDeclared.length > 0) {
    failures.push(`socket binds undeclared NEXT_S2C events: ${boundNotDeclared.join(", ")}`);
  }
  if (exposedNotDeclared.length > 0) {
    failures.push(`socket exposes undeclared NEXT_S2C events: ${exposedNotDeclared.join(", ")}`);
  }
