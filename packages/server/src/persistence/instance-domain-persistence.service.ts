 ...
 * 按域独立读写，支持增量刷盘和恢复水位。
 *
 * 模块级常量与游离函数已拆至 ./instance-domain-persistence.helpers.ts。
 */
...
import {
  ...
  buildCanonicalContainerSourceId,
  buildContainerMetadataPayload,
  buildPayload,
  buildStableDomainRowId,
  clampInteger,
  dedupeRecordRowsByKey,
  ensureBigintColumns,
  ensureDoubleColumns,
  ensureInstanceBuildingAuditLogTable,
  ensureInstanceBuildingCellTable,
  ensureInstanceBuildingOperationIdempotencyTable,
  ensureInstanceBuildingStateTable,
  ensureInstanceCheckpointTable,
  ensureInstanceContainerEntryTable,
  ensureInstanceContainerStateTable,
  ensureInstanceContainerTimerTable,
  ensureInstanceEventStateTable,
  ensureInstanceFengShuiStateTable,
  ensureInstanceGroundItemTable,
  ensureInstanceMonsterRuntimeStateTable,
  ensureInstanceOverlayChunkTable,
  ensureInstanceRecoveryWatermarkTable,
  ensureInstanceRoomCellTable,
  ensureInstanceRoomStateTable,
  ensureInstanceTemporaryTileStateTable,
  ensureInstanceTileCellTable,
  ensureInstanceTileDamageStateTable,
  ensureInstanceTileResourceStateTable,
  hashString,
  INSTANCE_BUILDING_AUDIT_LOG_LOCK_KEY,
  INSTANCE_BUILDING_AUDIT_LOG_TABLE,
  INSTANCE_BUILDING_CELL_LOCK_KEY,
  INSTANCE_BUILDING_CELL_TABLE,
  INSTANCE_BUILDING_OPERATION_IDEMPOTENCY_LOCK_KEY,
  INSTANCE_BUILDING_OPERATION_IDEMPOTENCY_TABLE,
  INSTANCE_BUILDING_STATE_LOCK_KEY,
  INSTANCE_BUILDING_STATE_TABLE,
  INSTANCE_CHECKPOINT_LOCK_KEY,
  INSTANCE_CHECKPOINT_TABLE,
  INSTANCE_CONTAINER_ENTRY_LOCK_KEY,
  INSTANCE_CONTAINER_ENTRY_TABLE,
  INSTANCE_CONTAINER_STATE_LOCK_KEY,
  INSTANCE_CONTAINER_STATE_TABLE,
  INSTANCE_CONTAINER_TIMER_LOCK_KEY,
  INSTANCE_CONTAINER_TIMER_TABLE,
  INSTANCE_DOMAIN_BIGINT_COLUMNS_BY_TABLE,
  INSTANCE_DOMAIN_DOUBLE_COLUMNS_BY_TABLE,
  INSTANCE_EVENT_STATE_LOCK_KEY,
  INSTANCE_EVENT_STATE_TABLE,
  INSTANCE_FENGSHUI_STATE_LOCK_KEY,
  INSTANCE_FENGSHUI_STATE_TABLE,
  INSTANCE_GROUND_ITEM_LOCK_KEY,
  INSTANCE_GROUND_ITEM_TABLE,
  INSTANCE_MONSTER_RUNTIME_STATE_LOCK_KEY,
  INSTANCE_MONSTER_RUNTIME_STATE_TABLE,
  INSTANCE_OVERLAY_CHUNK_LOCK_KEY,
  INSTANCE_OVERLAY_CHUNK_TABLE,
  INSTANCE_RECOVERY_WATERMARK_LOCK_KEY,
  INSTANCE_RECOVERY_WATERMARK_TABLE,
  INSTANCE_ROOM_CELL_LOCK_KEY,
  INSTANCE_ROOM_CELL_TABLE,
  INSTANCE_ROOM_STATE_LOCK_KEY,
  INSTANCE_ROOM_STATE_TABLE,
  INSTANCE_TEMPORARY_TILE_STATE_LOCK_KEY,
  INSTANCE_TEMPORARY_TILE_STATE_TABLE,
  INSTANCE_TILE_CELL_LOCK_KEY,
  INSTANCE_TILE_CELL_TABLE,
  INSTANCE_TILE_DAMAGE_STATE_LOCK_KEY,
  INSTANCE_TILE_DAMAGE_STATE_TABLE,
  INSTANCE_TILE_RESOURCE_STATE_LOCK_KEY,
  INSTANCE_TILE_RESOURCE_STATE_LOCK_NAMESPACE,
  INSTANCE_TILE_RESOURCE_STATE_TABLE,
  normalizeBuildingCellPersistenceRows,
  normalizeBuildingPersistenceRow,
  normalizeDbTimestamp,
  normalizeFengShuiPersistenceRow,
  normalizeIntegerWithFallback,
  normalizeJsonObjectPayload,
  normalizeMonotonicVersion,
  normalizeNullableInteger,
  normalizeNullableNumber,
  normalizeNumberWithFallback,
  normalizeOptionalString,
  normalizePersistedItemPayload,
  normalizeRecordIntegerKey,
  normalizeRequiredString,
  normalizeRoomCellPersistenceRow,
  normalizeRoomPersistenceRow,
  normalizeRotation,
  normalizeStringArray,
  projectBuildingCellPersistenceRow,
  projectBuildingPersistenceRow,
  projectFengShuiPersistenceRow,
  projectRoomCellPersistenceRow,
  projectRoomPersistenceRow,
  resolveInstanceCheckpointVersion,
  resolveInstanceWatermarkVersion,
  resolvePersistedGroundExpireAt,
  rollbackQuietly,
  shouldReplaceContainerState,
  toRecord,
} from './instance-domain-persistence.helpers';
import {
  deleteTileDamageStatesImpl,
  loadRuntimeTileCellsImpl,
  loadTemporaryTileStatesImpl,
  loadTileDamageStatesImpl,
  loadTileResourceDiffsImpl,
  replaceRuntimeTileCellsImpl,
  replaceTemporaryTileStatesImpl,
  saveTileDamageDeltaBatchImpl,
  saveTileDamageDeltaImpl,
  saveTileDamageStatesImpl,
  saveTileResourceDeltaBatchImpl,
  saveTileResourceDeltaImpl,
  saveTileResourceDiffsImpl,
} from './instance-domain-persistence.tiles';
...
export class InstanceDomainPersistenceService implements OnModuleInit, OnModuleDestroy {
  ...
  /** @internal 模块拆分后由 tiles helper 访问 */
  pool: Pool | null = null;
  /** @internal 模块拆分后由 tiles helper 访问 */
  enabled = false;
  ...
  async saveTileResourceDiffs(
    ...
    return saveTileResourceDiffsImpl(this, instanceId, entries);
  }
  ...
  async saveTileResourceDelta(
    ...
    return saveTileResourceDeltaImpl(this, instanceId, upserts, deletes);
  }

  /** 加载指定实例的全部地块资源状态 */
  async loadTileResourceDiffs(instanceId: string): Promise<Array<{ resourceKey: string; tileIndex: number; value: number }>> {
    return loadTileResourceDiffsImpl(this, instanceId);
  }
  ...
  async replaceRuntimeTileCells(
    ...
    return replaceRuntimeTileCellsImpl(this, instanceId, entries);
  }
  ...
  async loadRuntimeTileCells(instanceId: string): Promise<Array<{
    ...
    return loadRuntimeTileCellsImpl(this, instanceId);
  }
  ...
  async saveTileDamageStates(
    ...
    return saveTileDamageStatesImpl(this, instanceId, entries, ledgerClaim);
  }

  /** 删除指定实例的地块破坏状态（可按 tileIndex 过滤） */
  async deleteTileDamageStates(instanceId: string, tileIndices: number[] | null = null): Promise<void> {
    return deleteTileDamageStatesImpl(this, instanceId, tileIndices);
  }
  ...
  async saveTileDamageDelta(
    ...
    return saveTileDamageDeltaImpl(this, instanceId, upserts, deletes);
  }
  ...
  async loadTileDamageStates(instanceId: string): Promise<Array<{
    ...
    return loadTileDamageStatesImpl(this, instanceId);
  }
  ...
  async replaceTemporaryTileStates(
    ...
    return replaceTemporaryTileStatesImpl(this, instanceId, entries, ledgerClaim);
  }
  ...
  async loadTemporaryTileStates(instanceId: string): Promise<Array<{
    ...
    return loadTemporaryTileStatesImpl(this, instanceId);
  }
  ...
  async saveTileDamageDeltaBatch(
    ...
    return saveTileDamageDeltaBatchImpl(this, batch);
  }
  ...
  async saveTileResourceDeltaBatch(
    ...
    return saveTileResourceDeltaBatchImpl(this, batch);
  }
