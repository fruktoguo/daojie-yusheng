 ...
 * 记录doc输出。
 */
const docOutput = path.join(repoRoot, "docs", "mainline-boundary-audit.md");
 ...
 */
  ...
  process.stdout.write(`[mainline boundary audit] report written to ${docOutput}\n`);
  process.stdout.write(`[mainline boundary audit] matched ${summary.matchedChecks}/${summary.totalChecks} checks, ${summary.totalHits} code hits across ${summary.categories.length} categories\n`);
}
 ...
 */
  ...
  lines.push("## 一句话结论");
  lines.push("");
  lines.push("- 这份报告只统计仓库里仍可见的旧数据边界、兼容边界与性能热点，不等于 replace-ready 失败，也不代表完整替换已完成。");
 ...
 */
  ...
  lines.push("- 报告由 `packages/server/src/tools/audit/mainline-boundary-audit.ts` 自动生成。");
