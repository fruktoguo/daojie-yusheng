 ...
 * 记录doc输出。
 */
const docOutput = path.join(repoRoot, "docs", "legacy-boundary-audit.md");
 ...
 */
  ...
  process.stdout.write(`[legacy boundary audit] report written to ${docOutput}\n`);
  process.stdout.write(`[legacy boundary audit] matched ${summary.matchedChecks}/${summary.totalChecks} checks, ${summary.totalHits} code hits across ${summary.categories.length} categories\n`);
}
 ...
 */
  ...
  lines.push("- 报告由 `packages/server/src/tools/audit/legacy-boundary-audit.ts` 自动生成。");
