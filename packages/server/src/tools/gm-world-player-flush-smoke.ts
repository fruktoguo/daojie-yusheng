...
async function main() {
  ...
  const payload = await service.flushPlayerPersistence('player:gm-flush');
  assert.deepEqual(payload, { ok: true });
  assert.deepEqual(flushed, ['player:gm-flush']);
  console.log(JSON.stringify({
    ok: true,
    case: 'gm-world-player-flush',
    flushed,
  }, null, 2));
}
