...
function createDormantDungeonMonsterInstance() {
  const base = createAdjacentMonsterInstance();
  return {
    ...base,
    meta: {
      instanceId: 'dungeon:test_map',
      kind: 'dungeon',
    },
    getMonster(runtimeId: string) {
      return {
        ...base.getMonster(runtimeId),
        engaged: false,
...
function testDormantDungeonMonsterRequiresExplicitEngage(): void {
  const player = createAutoBattlePlayer();
  player.instanceId = 'dungeon:test_map';
  player.combat.combatActionTick = 11;
  const playerRuntimeService = createPlayerRuntimeService(player);
  const threatService = new WorldRuntimeThreatService();
  threatService.addThreat(threatService.buildPlayerOwnerId(player.playerId), 'monster:1', {
    baseThreat: 100,
    distance: 1,
    now: 12,
  });
  const service = new WorldRuntimeAutoCombatService(playerRuntimeService as never, threatService);
  const instance = createDormantDungeonMonsterInstance();
  const deps = {
    resolveCurrentTickForPlayerId() {
      return 12;
    },
  ...
  const automaticCommand = service.buildAutoCombatCommand(instance as never, player as never, deps as never);
  assert.equal(automaticCommand, null, '副本怪物未受击时，自动战斗不得用新仇恨或残留仇恨主动开怪');
  assert.deepEqual(playerRuntimeService.log, [], '休眠门控不得偷偷改写玩家战斗目标');

  const combat = player.combat as unknown as Record<string, unknown>;
  combat.combatTargetId = 'monster:1';
  combat.combatTargetLocked = true;
  const explicitCommand = service.buildAutoCombatCommand(instance as never, player as never, deps as never);
  assert.equal(explicitCommand?.kind, 'basicAttack', '玩家明确锁定休眠副本怪后必须允许首次攻击');
  assert.equal(explicitCommand?.targetMonsterId, 'monster:1');
}
...
testDormantDungeonMonsterRequiresExplicitEngage();
