 ...
 */
    ...
    child.once('exit', (code, signal) => {
      ...
      if (code !== null && code !== 0) {
        process.stderr.write(`[protocol audit] isolated server exited unexpectedly: code=${code} signal=${signal ?? 'none'}\n`);
      }
    });
  });
}
...
export { loadUniqueItemIds, waitForState, createCaseRuntime, createRuntimeApi, createAuditedSocket, createAuditor, assertStaticProtocolEventSurface, measurePayloadBytes, waitForHealth, stopServer, startIsolatedServer, allocateFreePort, waitForValue, waitFor, delay, repoRoot, distRoot, packageRoot, AuditCollector, connectHello, stopIsolatedServer, postJson, fetchState, fetchTileState, deletePlayer, writeMarkdown, uniquePlayerId, hasDatabaseUrl };
