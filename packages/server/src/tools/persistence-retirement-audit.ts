...
const RULES: AuditRule[] = [
  {
    ...
    banned: [
      ...
      { pattern: 'SERVER_MAP_LEGACY_SNAPSHOT_RESTORE', reason: '旧真源退役后不得保留旧 map snapshot 恢复开关' },
    ],
  },
  {
  ...
  },
  {
    ...
    banned: [
      ...
      { pattern: 'LEGACY_MARKET_', reason: '坊市旧真源退役后不得保留旧 scope 回读优先级' },
    ],
  },
  {
  ...
  },
  {
    ...
    banned: [
      ...
      { pattern: 'server_sects_v1', reason: '宗门旧真源退役后不得保留旧全服文档 scope' },
    ],
  },
  {
...
function main(): void {
  ...
  const report = {
    ...
    checked,
    failures,
    answers: '旧真源退役静态边界：玩家、地图、GM、坊市、兑换码、建议和宗门主线不得继续使用旧整档快照或通用文档桶真源。',
    completionMapping: 'release:proof:persistence-retirement-audit',
  };
