...
async function main(): Promise<void> {
      ...
      JSON.stringify(
        {
          ...
          completionMapping: 'replace-ready:proof:with-db.instance-lease-runtime',
        },
        null,
        2,
      ),
  ...
  try {
    ...
    await cleanupInstanceRows(pool, [fenceInstanceId, takeoverInstanceId]);
      ...
      JSON.stringify(
        {
          ...
          fenceProof,
          takeoverProof,
          answers: 'with-db 下已验证实例 runtime 会认领 persistent instance lease、接管过期 lease，并在 lease 不再属于本节点时阻断 dirty map 写链',
          excludes: '不证明真实多节点 socket 导流、跨节点 transfer、过期 lease 自动接管、split-brain 双活或玩家迁移缓冲',
          completionMapping: 'replace-ready:proof:with-db.instance-lease-runtime',
        },
        null,
    ...
    await cleanupInstanceRows(pool, [fenceInstanceId, takeoverInstanceId]).catch(() => undefined);
