 ...
 */
    ...
    await authedRequestJson('/api/account/role-name', loginResult.accessToken, {
        method: 'POST',
        body: {
            roleName: nextRoleName,
        },
    });
    await authedRequestJson('/api/account/password', loginResult.accessToken, {
        method: 'POST',
        body: {
 ...
 */
    ...
    await expectRequestFailure('/api/auth/login', {
        method: 'POST',
        body: {
            loginName: accountName,
            password,
 ...
 * 记录relogin结果。
 */
    const reloginResult = await requestJson('/api/auth/login', {
        method: 'POST',
        body: {
            loginName: accountName,
...
/** lastError：定义该变量以承载业务值。 */
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        try {
            await authedRequestJson('/api/account/display-name', accessToken, {
                method: 'POST',
                body: {
