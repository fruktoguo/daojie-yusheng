 ...
 */
        ...
        socket.emit(shared_1.C2S.UseAction, {
...
function buildMonsterInstanceCandidates(preferredInstanceId) {
    ...
    if (match) {
        ...
        candidates.push(`${scope === 'public' ? 'real' : 'public'}:${templateId}`, raw);
    }
    else {
        candidates.push(`real:${raw}`, `public:${raw}`);
    }
...
function resolveImmediateEngageProof({ playerId, runtimeId, beforePlayer, beforeMonster, firstSelfDelta, firstWorldDelta, }) {
  // 关键分支按状态与边界条件处理，非法路径会被提前拦截。
    ...
    const threatArrowAdd = Array.isArray(firstWorldDelta?.threatArrowAdds)
        ? firstWorldDelta.threatArrowAdds.find((entry) => Array.isArray(entry)
            && ((entry[0] === playerId && entry[1] === runtimeId) || (entry[0] === runtimeId && entry[1] === playerId)))
        : null;
    if (threatArrowAdd) {
        return {
            kind: 'worldDeltaThreatArrowAdd',
            monsterKey: String(threatArrowAdd[0] ?? runtimeId),
            playerId: String(threatArrowAdd[1] ?? playerId),
        };
    }
    const threatArrowRemove = Array.isArray(firstWorldDelta?.threatArrowRemoves)
        ? firstWorldDelta.threatArrowRemoves.find((entry) => Array.isArray(entry)
            && ((entry[0] === playerId && entry[1] === runtimeId) || (entry[0] === runtimeId && entry[1] === playerId)))
        : null;
    if (threatArrowRemove) {
        return {
            kind: 'worldDeltaThreatArrowRemove',
            monsterKey: String(threatArrowRemove[0] ?? runtimeId),
            playerId: String(threatArrowRemove[1] ?? playerId),
        };
    }
    return null;
}
