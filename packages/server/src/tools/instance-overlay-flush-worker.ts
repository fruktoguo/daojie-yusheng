...
async function main(): Promise<void> {
  ...
  assertFullAppFlushWorkerAllowed('instance-overlay-flush-worker');
  ...
  const { InstanceOverlayFlushWorker } = await import('../runtime/world/worker/instance-overlay-flush.worker.js');
  const worker = app.get(InstanceOverlayFlushWorker);

  try {
    const workerId = process.env.SERVER_INSTANCE_OVERLAY_FLUSH_WORKER_ID?.trim()
      || `instance-overlay-worker-${randomUUID()}`;
    if (once) {
