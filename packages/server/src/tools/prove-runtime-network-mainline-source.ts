...
const TARGETS = [
  {
    ...
    description: "runtime 主链不直接读取旧工作树文件路径",
  },
  {
    id: "network",
    directory: "packages/server/src/network",
    description: "network 主链不直接读取旧工作树文件路径",
  },
];
...
function main() {
  ...
  if (failures.length > 0) {
    process.stderr.write("[runtime/network mainline-source proof] failed\n");
  ...
  process.stdout.write("[runtime/network mainline-source proof] passed\n");
