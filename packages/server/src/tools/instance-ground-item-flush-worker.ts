...
async function main(): Promise<void> {
  ...
  assertFullAppFlushWorkerAllowed('instance-ground-item-flush-worker');
  ...
  const { InstanceGroundItemFlushWorker } = await import('../runtime/world/worker/instance-ground-item-flush.worker.js');
  const worker = app.get(InstanceGroundItemFlushWorker);

  try {
    const workerId = process.env.SERVER_INSTANCE_GROUND_WORKER_ID?.trim() || `instance-ground-worker-${randomUUID()}`;
    if (once) {
        ...
        JSON.stringify(
          {
            ...
            processedCount,
            workerId,
            answers: 'instance ground item worker 可独立认领 instance_flush_ledger，并驱动现有 map snapshot 刷盘',
            excludes: '不证明多节点 worker 竞争、完整 dead-letter 或 Redis 唤醒',
            completionMapping: 'release:proof:with-db.instance-ground-item-flush-worker',
          },
          null,
          2,
        ),
