/**
 * 玩家运行时游离模块函数集合 — 钱包/离线收益/统计域。
 *
 * 从 player-runtime.helpers.ts 拆分而来，包含：
 * - 钱包类型规范化/余额保障/读取
 * - 背包物品计数/消耗
 * - 离线收益 session/snapshot/delta 构建
 * - 离线收益报告合并/规范化
 * - 玩家统计记录/日维度汇总/patch
 * - 钱包缓存同步
 */
...
import { normalizedOfflineGainSnapshots, normalizedOfflineGainReportPartsRecords, canonicalOfflineGainReportPartsRecords, offlineGainTechniqueIndexBySnapshot } from './player-runtime.constants';
export function normalizeWalletType(walletType) {
 const value = typeof walletType === 'string' ? walletType.trim() : '';
 return value.length > 0 ? value : '';
}
export function ensureWalletBalances(player) {
...
export function readWalletBalance(player, walletType) {
...
export function readInventoryItemCount(player, itemId) {
...
export function isWalletCacheItemId(itemId) {
 return typeof itemId === 'string' && itemId.trim() === 'spirit_stone';
}
export function syncWalletCacheFromInventory(player, changedItemId = null) {
...
export function consumeInventoryItemCount(items, itemId, count) {
