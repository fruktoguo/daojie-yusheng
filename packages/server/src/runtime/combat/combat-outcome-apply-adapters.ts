...
export function createPlayerOutcomeApplyAdapter(handlers: OutcomeHandlers = {}) {
 return ({ outcome, target, result, application, deps }: OutcomeApplyInput) => {
  ...
  const playerAfterDamage = deps?.playerRuntimeService?.getPlayer?.(targetPlayerId);
  const actuallyDefeated = playerAfterDamage ? playerAfterDamage.hp <= 0 : result?.defeated === true;
  if (targetPlayerId && actuallyDefeated && result?.applyDefeat !== false) {
