...
/** 谷神不死：拦截致死伤害并锁定生命为 1 点（持续 1 息，冷却以技能定义为准且不受冷却缩减影响）。 */
export function interceptLethalDamageWithDeathImmunity(player: any, damage: number, currentTick: number): { prevented: boolean; finalHp: number } {
 ...
 const skill = findEnabledPlayerSkill(player, SKILL_HEAVEN_DEATH_IMMUNITY);
 if (!skill) return { prevented: false, finalHp: Math.max(0, currentHp - normalizedDamage) };
 ...
 const cooldownTicks = Math.max(1, Math.trunc(Number(skill.cooldown) || COOLDOWN_HEAVEN_DEATH_IMMUNITY));
 ...
 player.combat.cooldownReadyTickBySkillId[SKILL_HEAVEN_DEATH_IMMUNITY] = currentTick + cooldownTicks;
