...
*/
 ...
 async dispatchDamagePlayer(playerId: string, amount: number, deps: any) {
  ...
  const currentTick = Number(deps.resolveCurrentTickForPlayerId?.(playerId) ?? 0);
  const updated = this.playerRuntimeService.applyDamage(playerId, amount, null, { currentTick });
