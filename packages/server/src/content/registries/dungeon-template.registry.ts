...
function normalizePresentation(raw: unknown, basePath: string): DungeonPresentationDefinition | undefined {
 ...
 const onCombatEngaged = normalizeSteps(raw.onCombatEngaged, `${basePath}.onCombatEngaged`);
 ...
 if (!onRunCreated && !onCombatEngaged && !onMonsterAction) return undefined;
 return {
  ...(onRunCreated ? { onRunCreated } : {}),
  ...(onCombatEngaged ? { onCombatEngaged } : {}),
