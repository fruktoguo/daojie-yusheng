...
let ContentTemplateRepository = ContentTemplateRepository_1 = class ContentTemplateRepository {
    ...
    loadAll() {
        ...
        this.logger.log(`Loaded ${this.itemTemplates.size} item templates, ${this.techniqueTemplates.size} techniques, ${this.monsterDropsByMonsterId.size} monster drop tables and ${this.starterInventoryEntries.length} starter entries`);
    }
    loadMonsterDrops() {
    ...
    normalizeMonsterSkills(raw, monsterId) {
        ...
        for (const entry of raw) {
            ...
            if (!skill) {
                this.logger.warn(`Monster ${monsterId} configured missing skill ${skillId}, ignored`);
                continue;
            }
