...
let PlayerIdentityPersistenceService = PlayerIdentityPersistenceService_1 = class PlayerIdentityPersistenceService {
    ...
    async onModuleInit() {
        ...
        if (!databaseUrl.trim()) {
            this.logger.log('Player identity persistence disabled: no SERVER_NEXT_DATABASE_URL/DATABASE_URL');
            return;
        }
        ...
        try {
            ...
            this.enabled = true;
            this.logger.log('Player identity persistence enabled via persistent_documents');
        }
        catch (error) {
            this.logger.error('Player identity persistence init failed, fallback to disabled mode', error instanceof Error ? error.stack : String(error));
