...
let WorldSessionReaperService = WorldSessionReaperService_1 = class WorldSessionReaperService {
    ...
    onModuleInit() {
        ...
        this.timer.unref();
        this.logger.log(`World session reaper started at ${SESSION_REAPER_INTERVAL_MS}ms interval`);
    }
    onModuleDestroy() {
        if (this.timer) {
        ...
        try {
            ...
            for (const binding of expiredBindings) {
                try {
                    ...
                    this.logger.error(`World session reap failed for ${binding.playerId}`, error instanceof Error ? error.stack : String(error));
                }
            }
        }
        catch (error) {
            this.logger.error('World session reap failed', error instanceof Error ? error.stack : String(error));
        }
        finally {
            this.running = false;
        }
