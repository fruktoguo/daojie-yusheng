...
const entitiesPath = path.join(clientRoot, 'src/game-map/renderer/pixi-renderer.entities.ts');
...
function extractImplFunction(source, name) {
  const startMarker = `export function ${name}Impl(`;
  const start = source.indexOf(startMarker);
  assert(start >= 0, `缺少 ${name}Impl 函数`);
...
const [rendererSource, entitiesSource, stateSource, geometrySource] = await Promise.all([
...
const syncMethod = extractImplFunction(entitiesSource, 'syncArtifactAura');
assert.match(syncMethod, /view\.artifactAuraCellSize !== cellSize/, '缩放变化时必须重建对应尺寸的相位帧');
assert.match(syncMethod, /self\.createArtifactAuraFrame\(cellSize, frameIndex\)/, '启用时必须一次创建相位帧');

const createFrameMethod = extractImplFunction(entitiesSource, 'createArtifactAuraFrame');
...
const frameUpdateMethod = extractImplFunction(entitiesSource, 'updateArtifactAuraFrame');
...
const motionMethod = extractImplFunction(entitiesSource, 'patchEntityMotion');
assert.match(motionMethod, /self\.updateArtifactAuraFrame\(view, now\);/, '实体动画必须只切换缓存相位');
assert.doesNotMatch(motionMethod, /drawArtifactAura|createArtifactAuraFrame/, '实体动画不得直接绘制法宝路径');

const destroyMethod = extractImplFunction(entitiesSource, 'destroyEntityView');
