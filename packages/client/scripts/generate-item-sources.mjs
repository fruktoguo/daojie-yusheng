 ...
 */
 ...
 const dungeonsDir = path.join(repoRoot, 'packages/server/data/content/dungeons');
 if (fs.existsSync(dungeonsDir)) {
  for (const file of fs.readdirSync(dungeonsDir)) {
   if (!file.endsWith('.json')) continue;
   try {
    const dungeon = JSON.parse(fs.readFileSync(path.join(dungeonsDir, file), 'utf8'));
    for (const room of dungeon.rooms ?? []) {
     if (!Array.isArray(room.bossDropTable) || !room.bossId) continue;
     const mapTemplateId = room.mapTemplateId ?? dungeon.mapTemplateId;
     const mapName = mapNameById.get(mapTemplateId) ?? dungeon.name ?? '副本';
     const monster = monsters.find((m) => m.id === room.bossId);
     const monsterName = monster?.name ?? dungeon.name ?? '副本领主';
     for (const drop of room.bossDropTable) {
      pushSource(sourceByItemId, drop.itemId, {
       kind: 'monster_drop',
       mapId: mapTemplateId,
       mapName,
       monsterId: room.bossId,
       monsterName,
       chance: typeof drop.chance === 'number' ? drop.chance : undefined,
       count: escapeNonFiniteInteger(drop.count) ?? 1,
      });
