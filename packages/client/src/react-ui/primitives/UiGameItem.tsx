/**
 * 本文件提供 React UI 的 UiGameItem 基础组件，用于复用面板内的视觉和交互片段。
 *
...
export function UiGameItem({
  ...
  const classes = ['react-ui-game-item'];
  if (active) {
    classes.push('is-active');
  }
  if (compactName) {
    classes.push('react-ui-game-item--compact-name');
  }
  if (gradeTone) {
    classes.push('react-ui-game-item--grade', `react-ui-game-item--grade-${gradeTone}`);
  }
  if (className) {
  ...
  return (
    <button {...props} type={props.type ?? 'button'} className={classes.join(' ')}>
      <div className="react-ui-game-item-head">
        <span className="react-ui-game-item-type">{typeLabel}</span>
        {quantity ? <span className="react-ui-game-item-count">{quantity}</span> : null}
      </div>
      <div className="react-ui-game-item-name">{name}</div>
      {chips.length > 0 ? (
      ...
      {gradeLabel ? <div className="react-ui-game-item-grade">{gradeLabel}</div> : null}
      {note ? <div className="react-ui-game-item-note">{note}</div> : null}
      {children}
      {actions ? <div className="react-ui-game-item-actions">{actions}</div> : null}
    </button>
  );
}
