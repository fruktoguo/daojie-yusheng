/**
 * 本文件提供 React UI 的 UiFieldRow 基础组件，用于复用面板内的视觉和交互片段。
 *
...
export function UiFieldRow({
  ...
  const classes = ['react-ui-field-row'];
  if (className) {
  ...
  return (
    <div className={classes.join(' ')}>
      <span className="react-ui-field-row-label">{label}</span>
      {children ? <div className="react-ui-field-row-content">{children}</div> : null}
      {value !== undefined ? <span className="react-ui-field-row-value">{value}</span> : null}
    </div>
  );
}
