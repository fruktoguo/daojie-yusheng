...
// Next 协议到达顺序不一致时，先暂存等待 bootstrap 合并
...
socket.onLeaderboard((data) => {
  worldSummaryStateSource.handleLeaderboard(data);
});
socket.onWorldSummary((data) => {
  worldSummaryStateSource.handleWorldSummary(data);
});
...
async function copyTextToClipboard(text: string): Promise<boolean> {
  try {
    ...
    // 某些浏览器或非安全上下文会拒绝 Clipboard API，这里回退到传统复制命令。
  }
...
function trimCurrentPathProgress() {
  ...
  // 当前位置已经脱离剩余路径时，直接清掉过期高亮，避免箭头继续指回旧起点。
