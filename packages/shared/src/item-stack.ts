 ...
 */
export const ITEM_INSTANCE_PAYLOAD_KEYS: readonly string[] = ['enhanceLevel', 'learnTechniqueId', 'learnTechniqueMaxLevel', 'grade', 'level'];

/** 规范化单个实例态字段值：数字取整、其余类型 JSON 序列化。 */
function normalizePayloadValue(key: string, value: unknown): string {
  if (value == null) {
    return key === 'enhanceLevel' || key === 'learnTechniqueMaxLevel' || key === 'level' ? '0' : '';
  }
