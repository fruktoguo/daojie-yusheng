...
本规范以仓库当前事实为准：`packages/*` 是唯一主线，`legacy/*` 已从工作树移除；仓库目标是把 next 主线收敛为可证明、可演练、可替换的商业级工程体系。
...
- `legacy/client`、`legacy/shared`、`legacy/server` 不再存在于当前工作树
- 需要旧规则或旧数据格式时，默认对照 `main` 分支同名 `packages/*` 或 `参考/main-packages-ref`
- 用户描述含糊时，优先理解为：
  - 修复 next 主线缺口
...
- 默认先读当前主线实现，再按需查 `main` 同名 `packages/*` 或 `参考/main-packages-ref` 对照
...
- `packages/*` 是唯一主线
- `legacy/*` 已从工作树移除
- 行为基线默认对照 `main` 分支同名 `packages/*`
- `参考/` 只作为外部参考或一次性输入，不是默认开发主线
- 非用户明确要求时，不新增旧线归档目录或旧线运行时依赖
...
3. 是否需要查 `main` 同名 `packages/*` 或 `参考/main-packages-ref` 作为基线；若需要，只查对应最小范围
