...
> 让玩家能用自然语言让 AI 生成合法、平衡、可运营的功法，同时让 AI 的表达自由度与策划完全一致。

## 1. 背景

### 1.1 设计目标

- **AI 自由度 = 策划自由度**：AI 能写的结构策划也能写，策划能写的结构 AI 也能写，两者共用同一条战斗结算路径
- **不改代码的前提下 AI/策划行为一致**：一次性重构数值层，之后 AI/策划没有分流
- **滥发数值的风险由机制消解，不由审核堵**：通过预算归一化让 AI 写多大的数字都不会破坏平衡
- **功法 JSON 瘦身**：从每层显式配置改为参数 + 全局公式展开

### 1.2 原系统阻碍 AI 生成的根因

| 问题 | 说明 |
|------|------|
| `formula` 表达式树 | AI 生成的嵌套 AST 需要 LLM 的 structured output 约束 |
| 技能强绑定功法 | 技能嵌套在功法 `layers[].skills[]`，难独立生成 |
| 纯静态 JSON 配置 | 无持久化层，AI 生成的功法无处存放 |
| 每层显式 `attrs` + `expFactor` | 每个功法展开成 80+ 行 JSON，数值分散难统一 |
| 每功法独立经验曲线 | 现有曲线混乱（反推 difficulty 在 0.15~3.84 之间），无全局规律 |

### 1.3 关键认知转变

现代 LLM（GPT-4o、Claude 3.5+、Gemini 1.5+ 等）支持 JSON Schema constrained decoding，**"AI 难以生成嵌套 AST"这个旧前提已不成立**。防线可以从输入侧（限制表达力）完全后移到**机制侧（归一化）+ 输出侧（多层校验）**。
