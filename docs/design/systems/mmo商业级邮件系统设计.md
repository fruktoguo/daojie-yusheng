...
定义商业级 MMO 邮件系统的目标架构：拆分服务端职责、持久化真源、协议和运营链路。

## 当前状态（2026-04-22）

已落地：
- `player_mail / player_mail_attachment / player_mail_counter` 结构化表接入读写主链
- 附件领取接入 `DurableOperationService`，联动 `durable_operation_log / outbox_event / asset_audit_log`
- GM 数据库备份/恢复覆盖邮件结构化主线表

未完成：
- `mail_campaigns / mail_messages / mail_receipts` 运营级批次和回执真源
- GM 广播异步任务投递（当前是请求线程内逐玩家串行）
- 在线新邮件 push、失败重试、运营报表、批次审计
