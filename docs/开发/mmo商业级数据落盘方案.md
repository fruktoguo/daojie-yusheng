...
本文档描述 `道劫余生` 当前生产主线面向商业级 MMO 的数据落盘终局方案。
...
2026-04-28 本轮实现已经完成旧业务真源硬切的核心落地：玩家和地图旧快照退出正式 runtime，低频业务状态迁到专表，资产奖励入口缺 durable 条件时硬失败，并新增 `persistence-retirement-audit` 静态门禁。本文仍按终局方案描述，容量压测、迁移演练和 `shadow / acceptance / full` 属于计划文档里的商用级证明项。
