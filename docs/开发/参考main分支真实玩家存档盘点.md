...
- 这份文档盘点的是参考项目 `/home/yuohira/mud-mmo` 的 `main` 主线真实玩家存档面
- 它不是在声明 `mud-mmo-next` 已经把这些域全部实现成结构化真源表
- `mud-mmo-next` 当前已落地到哪一步，应以 [main主线玩家数据分表方案.md](./main主线玩家数据分表方案.md) 和 [商业级数据落盘改造计划.md](./计划/商业级数据落盘改造计划.md) 的“当前实现映射/当前落地快照”为准
