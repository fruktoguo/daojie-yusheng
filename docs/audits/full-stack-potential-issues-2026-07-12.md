...
| 性能、内存、网络包体 | 进行中 | 文件体积门禁只剩 10 个历史 baseline 增幅；本轮发现的无 baseline 新超限已全部拆回阈值内，自动生成数据也已与手写职责门禁分离；构建产物仍有大 chunk 警告 | 继续逐个处理 10 个真实基线增幅，并区分热路径问题、冷路径债务和可独立职责 |
...
- 严重级别：高。
- 根本原因：多个运行时、持久化、GM 和客户端面板持续把新职责并回巨型文件；审计开始时体积门禁报告 14 个已超限文件继续增长，另有 2 个文件首次超过 3000 行。生成文件和大型 smoke 又与生产模块混在同一口径，增加了噪音。
...
- 当前证据：`pnpm proof:file-size-gate` 仍因 10 个真实手写模块的 baseline regression 退出 1；本轮发现的战斗 action、玩家成长、宗门 runtime、GM 玩家服务、协议审计、玩家分域 smoke、背包面板和 Pixi renderer 等无 baseline 新超限均已拆回 3000 行内，当前没有新超限或陈旧 baseline。
...
- 本轮进展：技能管理子面板原先只有公开入口和少量状态编排，两个弹层模板、全部预设动作、草稿变更及事件绑定仍通过非公开类型断言反向调用 `ActionPanel`。现已把完整技能管理/预设职责迁入 `SkillManagementSubpanel`，主面板只保留通用行动列表和子面板所需的显式 port；`action-panel.ts` 从门禁口径 4173 行进一步降到 2251 行并删除旧 baseline，子面板为 1655 行。当前仍有 11 个其他真实 baseline regression，FS-002 因此继续保持未完成。
- 本轮进展：`craft-workbench-modal.ts` 已把强化实际模板、事件、Tooltip 和历史弹层委托给 `CraftEnhancementView`，却继续保留约 2400 行不可达副本及另一套仍被更新链调用的历史实现。现已让强化子视图唯一拥有上述职责和临时 UI 清理，删除父类副本；主文件从 6144 行降到门禁口径 3853 行，强化子视图为 1713 行，历史增长回归从 11 个降为 10 个。主文件仍超过 3000 行，既有 4062 行 baseline 暂不删除，后续需继续拆出传功与功法精炼职责。
- 本轮进展：传功、功法书分解与抄录的模板、局部 patch、筛选、选择、确认和事件已提取到门禁口径 1035 行的 `CraftTransmissionView`，工坊主类只保留模式编排和显式状态端口；`craft-workbench-modal.ts` 进一步降到门禁口径 2948 行并退出硬阈值，4062 行旧 baseline 已删除。该 baseline 在删除前已经没有增长，因此真实 baseline regression 仍为 10 个，FS-002 继续保持未完成。
...
### FS-064 `[x]` 语言包生成物携带未消费元数据且被职责体积门禁误判

- 严重级别：中。
- 根本原因：`generate-i18n.mjs` 除客户端唯一消费的 `CLIENT_I18N_MESSAGES` 和 key 类型外，还把每条 CSV 的 category 与非空 note 复制成 `CLIENT_I18N_CATEGORIES`、`CLIENT_I18N_NOTES`；全仓没有任何消费点。文件体积门禁又无条件扫描所有 `.ts/.tsx`，把随内容条数线性增长的 `i18n.generated.ts` 纳入手写模块 baseline。
- 为什么错误：category/note 是内容维护元数据，真源应留在 CSV，不应进入客户端 TypeScript 图；自动生成数据的行数表达内容规模，而非模块职责或控制流复杂度，用同一 3000 行阈值会持续制造不可行动的假红。
- 触发条件：运行任意 client prebuild/typecheck 时都会解析多余导出；当 CSV 继续增加文案、生成文件超过旧 baseline 时，体积门禁即使没有新增手写职责也会失败。
- 后果：每次 typecheck/build 都需额外解析数千行不会被使用的对象字面量；新增正常文案也会让架构门禁失败，维护者容易错误扩大 baseline，从而掩盖真正继续膨胀的 runtime、持久化和 UI 模块。
...
- 验证：client TypeScript、高频 UI continuity proof 与完整 `pnpm verify:client` 通过；新增源码守卫确认 combat/sect 打开态刷新委托唯一子面板，并禁止主类重新出现战斗设置模板、宗门模板、自动丹药旧配置和宗门解析器。`pnpm proof:file-size-gate` 确认主文件为 4173 行、回到 5631 行 baseline 内，真实增长项从 12 个降为 11 个。其后发现的技能管理活跃反向委托已由 FS-066 独立修复，不改变本项对不可达旧实现的结论。

### FS-066 `[x]` 技能管理子面板反向调用主类私有实现，职责拆分名存实亡

- 严重级别：中高。
- 根本原因：`SkillManagementSubpanel` 已接管打开和刷新入口，但管理弹层、预设弹层、草稿变更、排序拖拽、导入导出和事件绑定仍保留在 `ActionPanel`；子面板通过 `as unknown as { _render... }` 一类非公开类型断言反向调用主类私有方法。`ActionPanelInternal` 还把草稿变更回调作为父类 port 暴露给子面板。
- 为什么错误：子面板既不能独立拥有自己的 UI 生命周期，又绕过 TypeScript 可见性建立隐式双向依赖；任何私有方法改名、状态迁移或模板调整都可能只在运行时暴露。主面板继续聚合两个弹层的状态机和 DOM 事件，也使“已拆分”的边界无法降低修改面与回归范围。
- 触发条件：修改技能预设导入/导出、自动/手动技能排序、拖拽、草稿应用或弹层关闭逻辑；编译器无法验证反向断言中声明的私有契约是否仍与父类实现一致。
- 后果：技能配置可能出现按钮无响应、事件重复绑定、关闭时漏掉未应用草稿确认、重绘后 tooltip 丢失，或者修复只落在父类/子类其中一侧；主面板长期保持巨型并阻碍局部 UI 验证。
- 修复方式：让 `SkillManagementSubpanel` 唯一拥有两个弹层模板、事件绑定、草稿变更、排序拖拽和预设全生命周期；`ActionPanelInternal` 只提供共享状态、通用技能配置投影及外部回调，不再暴露反向实现 port。删除主类中 71 个已不可达的技能管理/预设私有方法，并保留打开态局部重绘、滚动恢复、未应用草稿确认和 tooltip 重绑语义。
- 验证：client TypeScript、高频 UI continuity proof 与完整 `pnpm verify:client` 通过；源码守卫确认主面板只委托两个刷新入口、子面板直接拥有两个 owner、关闭确认/tooltip/预设导入入口仍在，并禁止双方恢复私有反向调用。`pnpm proof:file-size-gate` 确认主面板为 2251 行、子面板为 1655 行，无新超限或陈旧 baseline，完整门禁仍按预期只因其余 11 个真实增长项退出 1。

### FS-067 `[x]` 工坊强化子视图接管后父类仍保留双实现且关闭时清理错误实例

- 严重级别：中高。
- 根本原因：`CraftEnhancementView` 已经负责生产路径中的强化模板、事件、公式 Tooltip 和历史弹层，但 `CraftWorkbenchModal` 仍保留一整套不可达强化模板与另一套仍被面板回包调用的历史合并/刷新实现；父类还创建了一个从未绑定到 DOM 的 `enhancementFormulaTooltip`，关闭时隐藏的是这个废弃实例。炼丹材料选择弹层也没有进入 `clear` 和两种工坊宿主的关闭清理。
- 为什么错误：同一状态被两个类用两套实现解释，更新链与显示链没有唯一拥有者，维护者可能修到不会执行的副本；生命周期清理必须命中实际持有监听和固定态的对象，隐藏一个未使用实例不能释放子视图中已经固定的 Tooltip。材料弹层依附工坊会话，却不随会话结束释放，同样违反弹层拥有权边界。
- 触发条件：强化公式 Tooltip 以触控或点击固定后退出工坊或登出；打开炼丹材料选择器后关闭工坊；强化记录 patch 到达时，父类历史实现刷新一个由子视图打开的弹层；后续开发修改父类残留模板而误以为会影响生产 UI。
- 后果：强化 Tooltip 或材料选择器可跨工坊会话残留、遮挡其他界面；历史列表和详情可能由不同实现交替重建，增加状态漂移和回归概率；6144 行父类的伪拆分继续放大 review、事件重复绑定和局部刷新风险。
- 修复方式：强化记录回包、选中态校正和已打开历史弹层刷新统一委托 `CraftEnhancementView`；子视图新增唯一 `closeTransientUi`，释放选择器、三级历史弹层和实际 Tooltip，并接管强化刷新事件；父类清理炼丹材料选择器及其会话草稿，只保留协议合并和工坊模式编排，删除重复模板、历史、选择器、Tooltip、辅助类型与无用依赖。
- 验证：client TypeScript 与高频 UI continuity proof 通过；源码守卫确认父类不会重新吸收强化运行态、历史、选择器和 Tooltip，实现仍保留专用强化 active job 而非退化到公共队列。`pnpm proof:file-size-gate` 确认主文件为 3853 行、子视图为 1713 行，历史增长回归从 11 个降为 10 个；完整 `pnpm verify:client` 见验证表。

### FS-068 `[x]` 传功与功法精炼局部刷新 key 漏掉显示和成本语义且职责仍堆在工坊主类

- 严重级别：中高。
- 根本原因：传功结构 key 只拼接功法 ID、任务状态和附近玩家 ID，功法书抄录 key 也只记录 ID、品阶、类型和最高层；实际模板却同时消费功法名、境界、附近玩家名、待领悟名称/规则和境界参与的残页成本。约 900 行传功、分解、抄录模板和事件仍直接放在 `CraftWorkbenchModal`，被聚焦输入延迟的结构 patch 也没有在焦点离开后主动补做。
- 为什么错误：局部 patch 以 semantic key 判断“是否需要重建结构”，key 必须覆盖模板和公式读取的所有非高频字段；同一 ID 不代表名称、境界、规则和成本不变。输入聚焦时延迟重建是保护输入连续性的必要行为，但焦点离开后如果没有新状态包，延迟不能永久变成漏更新。主类同时拥有四种工坊子系统的模板与本地状态，也让已有子视图边界名存实亡。
- 触发条件：自创功法在 ID 不变时更新名称、品阶、类型或境界；附近玩家改名但仍是同一 playerId；待领悟记录改变名称、所需进度或自行领悟规则；功法境界改变导致抄录残页成本变化；上述结构变化发生时玩家正聚焦传功搜索框或下拉框，随后没有其他状态包到达。
- 后果：传功下拉框和附近玩家列表继续显示旧文本，待领悟状态可能保留旧结构；抄录面板可展示按旧境界计算的残页成本，玩家据此提交后会与服务端权威结果不一致；焦点离开后界面仍可能长期停留在旧结构。巨型父类继续扩大事件重复绑定、关闭清理和局部更新回归面。
- 修复方式：新增 `CraftTransmissionView`，唯一拥有传功、功法书分解/抄录的模板、筛选、选择、确认弹层、事件和局部 patch；父类只传入玩家/背包状态与回调并统一释放子视图弹层。传功 key 纳入功法名称/品阶/类型/境界、待领悟结构和玩家名称，抄录 key 纳入名称/境界/最高层，功法书 key 纳入 `learnTechniqueId`；保留聚焦期只 patch 进度，并在 `focusout` 后补做结构 patch。
- 验证：client TypeScript、`proof:craft-panel-local-patch` 和高频 UI continuity proof 通过；源码守卫确认主类不再持有传功/精炼模板与事件，并锁定输入聚焦保护、焦点离开补偿和完整 semantic key。文件体积门禁确认主类 2948 行、子视图 1035 行并成功删除旧 baseline；完整 `pnpm verify:client` 见验证表。运行完整体积门禁时，共享工作区另有不属于本组的未提交宗门 runtime 变更达到 3034 行，因此当前工作区仍额外报告一个新超限，本组未覆盖或提交该并行改动。
...
### D-010 `[?]` 强化历史中的成功率按当前条件重算，不能还原行动发生时的实际概率

- 当前证据：共享 `PlayerEnhancementLevelRecord` 只保存目标等级、成功次数和失败次数；服务端 `touchEnhancementLevelRecord` 只累加这三项，数据库 `levels_payload` 原样保存该数组。客户端打开历史详情时用当前角色强化等级、当前强化锤、当前幸运和物品当前模板等级重新执行 `computeEnhancementAdjustedSuccessRate`。界面文案已经明确标注“按当前角色强化等级与当前强化锤修正计算，仅用于参考”，因此现有实现与当前文案一致，不能直接判定为技术 bug。
- 潜在后果：玩家更换强化锤、幸运或强化等级变化，甚至后续调整公式或物品等级后，同一条历史会显示不同概率；运营无法用该界面审计某次实际尝试采用了什么概率，也无法准确核对争议结算。若产品只需要“用当前条件模拟这段等级区间”，现状反而避免了为每次尝试永久保存概率快照的额外存储和协议成本。
- 无法直接确定的原因：“历史”究竟是结算审计记录，还是成功/失败次数加当前条件参考表，属于产品语义；保存单一等级概率、每次尝试概率，或按会话保存技能/工具/幸运/公式版本，精度、存储量和旧记录兼容含义均不同。
- 可选方案：① 推荐：若需要可追责历史，为每个结算等级保存实际尝试次数对应的概率统计或逐次快照，并记录公式版本及当时技能/工具/幸运来源；详情明确区分“历史实际概率”和“当前参考概率”；② 只保存会话开始时的技能、工具、幸运和公式版本，成本较低，但会话中工具或状态变化时仍需定义冻结规则；③ 保持现状，把功能名称与说明进一步明确为“强化次数记录与当前概率参考”，不承诺历史概率审计。
- 2026-07-13 需要决定：强化记录是否承担运营审计和玩家争议核对职责，以及需要会话级还是逐次结算级精度；决定后再同步调整 shared 契约、服务端记录、持久化 JSON、客户端展示和旧记录语义。
...
| 行动面板旧实现清理 | client TypeScript、高频 UI continuity proof 与完整 `pnpm verify:client` 通过；本项提交时文件体积门禁确认主文件 4173 行并仅剩 11 个真实增长项 | combat/sect/preset 不可达代码岛删除，打开态刷新仍委托唯一子面板，主文件回到既有 baseline 内 | 本项当时残留的技能管理反向委托已由 FS-066 修复；未做真实弹层视觉/触控回归 |
| 技能管理子面板职责收口 | client TypeScript、高频 UI continuity proof 与完整 `pnpm verify:client` 通过；文件体积门禁按预期只剩其他 11 个真实增长项 | 两个弹层、草稿、排序拖拽、事件和预设生命周期由子面板唯一拥有；主面板降至 2251 行并移除旧 baseline | 未做真实浏览器弹层视觉、键盘/触控拖拽和导入文件回归；其余 11 个 baseline regression 未处理 |
| 工坊强化子视图职责与关闭生命周期 | client TypeScript、高频 UI continuity proof 与完整 `pnpm verify:client` 通过；文件体积门禁按预期只剩其他 10 个真实增长项 | 强化 active job、历史弹层、刷新事件和实际 Tooltip 由单一子视图拥有；父类清理完整会话弹层，主文件降至 3853 行 | 未做真实浏览器触控固定 Tooltip、弹层层叠和关闭动画回归；主文件仍超过 3000 行并保留旧 baseline |
| 传功与功法精炼子视图及 semantic key | client TypeScript、工坊局部 patch proof、高频 UI continuity proof 与完整 `pnpm verify:client` 通过；体积门禁确认工坊主类退出硬阈值 | 传功/分解/抄录由唯一子视图拥有，结构 key 覆盖名称、境界和成本语义，输入焦点离开后补偿延迟刷新；主文件 2948 行并删除 baseline | 未做真实浏览器长输入、附近玩家改名和服务端成本并发变更回归；共享工作区的并行宗门改动使全量体积门禁额外报告一个非本组新超限 |
