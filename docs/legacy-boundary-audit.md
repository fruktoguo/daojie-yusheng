...
| P1 runtime/persistence compat | 1 / 4 | 1 |
| 目标差距: 性能/扩展 | 0 / 5 | 0 |
...
- 运行命令：`pnpm audit:boundaries` 或 `pnpm --filter @mud/server audit:legacy-boundaries`。
- 报告由 `packages/server/src/tools/audit/legacy-boundary-audit.ts` 自动生成。
