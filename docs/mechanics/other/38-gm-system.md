...
- 恢复前先刷盘玩家与地图，清理会话、路由和运行时缓存，再以单一事务替换 `public` schema。
- 自动 checkpoint 完成后先在内存中保护当前备份元数据；新 schema 建立后以事务回填文件索引和 `checksumSha256`，保证源包与回滚点仍可再次恢复。
- 恢复 SQL 提交后保持 `SERVER_RUNTIME_RESTORE_ACTIVE=1`，先完成任务状态与审计落库，再直接以退出码 `0` 结束当前进程，由外部守护进程重启。
- 恢复已提交后禁止走 `SIGTERM` 普通优雅关服；该链路的最终 flush 携带恢复前的旧内存态，会反向覆盖新数据库。
- 重启后的新进程只从恢复后的数据库重建世界、玩家、市场和鉴权运行态。
