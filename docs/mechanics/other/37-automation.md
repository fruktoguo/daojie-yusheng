...
- socket 断开只解绑会话，不关闭自动战斗、自动反击、打坐修炼，也不清空采矿等通用技艺 job
- 离线角色仍在所属地图实例的权威 tick 名单中，自动战斗、玩家成长 tick 和技艺 job tick 使用同一份运行态继续推进
- detached runtime 回收会保留正在执行的自动战斗、打坐修炼、技艺 job 与队列
- 数据库持久化启用时，有效离线收益会话还会保留等待启动的自动打坐、已有目标的自动反击或待执行交战意图，避免回收窗口早于下一次有效 tick
- 离线挂机达到时长上限、角色死亡、目标失效或 job 正常完成/取消后，按各自机制结束，不额外延长无效运行态
