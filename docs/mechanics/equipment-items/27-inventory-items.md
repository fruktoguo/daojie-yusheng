...
- 客户端发送的 `knownRevision` 是版本下界：服务端运行态和客户端接收的分页都不得低于该版本。背包增量推进 revision 时，必须原地补丁当前页（按 `itemInstanceId` 回绑，没有实例 ID 时才按槽位+`itemId`），更新标题占用数与可见总数；不得丢掉当前分页快照，也不得退回本地全量第一页。revision 变化不得取消仍在飞行的分页请求。旧 revision 回包按 `invalid-current` 拒绝后仍保留当前页，再合并刷新。筛选、搜索与用户翻页仍立即请求。
...
- `staminaAmount`: 固定恢复副本体力；当前回元丹每枚恢复 24 点，上限仍为 240，使用时与其他消耗品一样扣除背包数量并标记 progression 域持久化。
...
- 邀请者的境界达成额外奖励使用永久邮件补发：受邀者达到炼气期补 1 枚 `wudao_yujian`，达到筑基期再补 3 枚；历史已达标记录由启动期和活动面板访问时按引渡记录幂等补发。
