...
- 玩家刷盘 payload 的 `itemInstanceId` 若已被其他玩家数据库行占用，必须先隔离，禁止直接改写现有归属。仅当 `itemId`、完整持久化实例态和非锁定状态都一致时，恢复链才可为待重放 payload 换发新 UUID 并重新入队；若旧会话 fence 已被新会话取代，只有在启动前、玩家离线且 inventory 领域版本仍高于数据库水位时才可去除旧 fence 后重放，普通 worker、关机 drain 或在线玩家仍继续隔离；任一条件不一致时等待人工核对。
