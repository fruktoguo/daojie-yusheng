...
- 背包与装备槽共同构成一次资产变更；穿戴、替换和卸下必须按玩家串行执行。
- 正式持久化启用时，服务端先通过 durable operation 在同一数据库事务内提交背包与装备槽，并同步推进两者 recovery watermark；提交成功后才更新运行态和客户端增量。
- 同槽位替换时，原装备按完整物品签名返回背包。不同 `itemId` 的装备不得合并；同模板、同实例态装备可以合并数量，但拆出单件穿戴时必须分配新的 `itemInstanceId`。
- 事务失败或会话、实例 lease 围栏失效时不修改运行态，避免断线、重连、跨实例或分域刷盘留下半笔换装结果。
