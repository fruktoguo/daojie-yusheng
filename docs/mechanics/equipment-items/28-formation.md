...
- 每息维护的基础灵力消耗为自身 `maxQiOutputPerTick` 向下取整，最低 1 点；若玩家拥有 `formation.speedRate`，则按 `floor(baseRate × (1 + speedRate))` 提升维护速度，最终仍至少消耗 1 点。
...
- 从挖矿、采集等其他技艺切换到阵法维护时，首个阵法资产 tick 必须先在同一玩家资产锁内把新的 `active_job` 刷入真源；写入 flush ledger 只代表持久化义务已暂存，不代表数据库真源已收敛，必须按当前域修订确认真实落库，必要时强制提交当前快照。若旧任务尚未收敛则本息失败关闭并保留重试，禁止放宽 `jobRunId + jobVersion` CAS 覆盖旧任务。
...
- terrain_stabilizer: 稳定地形，防止破坏
  ...
  - 晶精生成的人工矿脉最多持续 86400 实例息，固脉不能延长其寿命；受损期间仍按人工地块获得固脉回血与减伤。
...
- monster_suppression: 封魔压制
  ...
  - 每层对妖兽主要战斗属性提供 `-1%` 负向百分比修正，有效削弱百分比最高按 `100%` 结算；共享 `percentModifierToMultiplier` 反比衰减后，属性最低保留 `1 / (1 + 100 / 100) = 1/2`。
  - 多个封魔阵重叠只取最高层，不叠加；超过 100 层的强度不再继续降低属性。
  - 击杀经验按同一有效剩余乘区结算，最低保留 `1/2`。
