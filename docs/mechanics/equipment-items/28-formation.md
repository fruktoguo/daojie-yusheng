...
- 每息维护属于玩家灵力向阵法灵力池的资产转换：玩家 `vitals`、阵法技艺 `profession`、`active_job` 与 `instance_formation_state` 必须在同一 Durable Operation 中提交，并同时校验玩家 session、实例 node/token/epoch/expiry、job version 与阵法 `updatedAt`；提交成功前不得向运行态暴露扣灵、阵法增量、经验或 job 进度。
