...
| 人榜 | 灵石 | 背包灵石 + 坊市托管灵石 + 求购/竞拍冻结灵石 + 未领取邮件附件 + 创建者名下宝库灵石 | `spiritStones` |
...
- 灵石榜按玩家背包灵石、坊市托管仓灵石、开放求购单预留灵石、当前竞拍冻结灵石、仍可领取的邮件灵石附件，以及其创建的全部宝库灵石之和排序。宝库内灵石无论由谁存入，均归属宝库创建者；已领取、软删除或过期邮件不再计入。
- 钱包外资产由排行榜低频统一汇总，个人榜按玩家归属合并；世界摘要另补没有有效玩家快照或没有创建者归属的资产差额，禁止重复相加。
- 数据库异常残留在背包锁定空间的灵石仍按所属玩家计入统计；锁定状态本身不因此解除，也不会变为可操作物品。
...
interface WorldSummary {
  totalSpiritStones: number;      // 全服灵石总量（背包 + 坊市托管/冻结 + 未领取邮件附件 + 宝库）
  actionCounts: {
