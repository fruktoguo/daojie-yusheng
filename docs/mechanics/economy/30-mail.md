...
- 附件类型: 物品（含数量）或灵石
- 灵石附件同样进入 `player_inventory_item` 背包真源；`player_wallet` 仅由领取后的最终背包精确投影，禁止使用旧钱包余额做增量累加。
- 当前仍可领取的灵石附件按收件人归属计入个人灵石榜与世界灵石总量；已领取、软删除或过期邮件不计入。
- 单个可合并堆领取后不得超过 `2_147_483_647`；超过上限时整批拒绝，邮件保持未领取。
...
- 批量领取把全部附件、背包真源、钱包投影、邮件/附件领取态、邮箱计数、水位、outbox 与 audit 放入同一事务；任一附件容量或数量越界时不提交部分结果。
