...
- 浏览列表支持一级分类（全部/装备/功法书/材料/消耗品等 `ItemType`）与二级子 tab 筛选，过滤与计数全部由服务端在分页前完成，客户端只展示当前页。
- 二级分类由 `buildMarketListingSubType` 按物品模板字段推导，不存在坊市私有的子类型配置：装备取 `equipSlot`，功法书取功法分类，材料按 `itemId` 前缀分 herb/special，消耗品由 `resolveMarketConsumableCategory` 按模板 `tags` 推导——`丹药/药品/药散/药膏/战斗丹药/基础药品` → pill（丹药），`阵盘/符箓/符` → formation（符阵），`种子` → plant（灵植），其余 → other（其他）。
- 请求字段 `consumableCategory` 非法值在服务端 normalize 为 `all`；响应回显规范化后的值供客户端丢弃过期回包，各子类数量随 `counts.consumableCategoryCounts` 返回。
...
回元丹每天限购 5 个，按北京时间自然日重置。限购计数与功德扣除、物品发放在同一市场强事务内提交，失败或重放不会重复占用额度。
