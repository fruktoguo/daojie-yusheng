...
顶部行动队列悬浮界面为每个任务提供“置顶、下移、移除”三个快捷按钮。当前 active job 只能移除（即提交取消意图），不参与等待顺序调整；等待和休眠任务可以移动到等待队首、向队尾方向移动一位或直接移除。排序只提交 `queueId + action` 的服务端意图，由玩家 pending-command 串行执行；客户端不做权威乐观重排。有效顺序变更必须标记 `active_job` dirty，按队列数组顺序写回 `queue_order`，并在断线重连或服务重启后恢复。陈旧 ID、已在队首置顶、已在队尾下移均按幂等无变化处理。

悬浮队列的任务增删、状态或顺序变化才允许替换列表结构；每息进度必须按稳定任务 ID 原位更新文本和进度条，不能重建整个悬浮列表或重复绑定按钮事件。
...
大范围技能同次命中多个矿脉时，挖矿经验仍按目标顺序逐次采用当时的挖矿等级，并以当前地图等级作为 `targetLevel`，保持每次正收益至少为 1、逐级升级和单次境界修为取整规则；实现上缓存相同“挖矿等级 + 地图等级”的经验结果，最后一次写回技能状态并合并境界修为落账。

### 技艺被动功法获取

炼丹、锻造、强化、传法、采集、挖矿、建造和阵法维护每完成一次有效技艺动作，均有机会获得对应技艺的被动功法。候选档位严格由本次动作的 `targetLevel`（行动等级）决定：
...
低等级动作不能越级获得高档位，高等级动作也不会回落到低档位。单次动作的基础获取概率按原始（未经过速度修正）动作时长反推：

```ts
expectedTicks = skillLevel × 3^gradeIndex × 4 × 3600
baseProbability = baseActionTicks / expectedTicks
probability = clamp(baseProbability × (1 + luck × 0.01), 0, 1)
```

`gradeIndex` 使用功法品阶顺序的零基索引（凡人 0、黄阶 1、玄阶 2），因此技能 1 级时三档期望时间分别为 4、12、36 小时。`baseActionTicks` 必须沿用技艺经验计算所使用的原始动作时长；速度、装备和被动加成只影响实际推进速度，不改变该基准。幸运取玩家最终有效幸运值并作为倍率增幅。
