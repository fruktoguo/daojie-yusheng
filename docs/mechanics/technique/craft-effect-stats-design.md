...
7. 产出率只影响结果数量、掉落、副产物、返材、省耗、收获等，不直接修改成功判定。普通数量型产出按“固定额外数量 + 小数概率额外 1 个”结算，例如基础 1 个、`outputRate=3.5` 时固定额外 3 个，并有 50% 概率再额外 1 个；矿物受击掉落是明确例外，`mining.outputRate` 先作为独立乘区并入最终期望数量，再转换为最高 10% 的触发率和随机掉落数量，避免成功后重复放大。
...
| 挖矿 `mining` | 当前矿脉受击固定进入掉落期望曲线；基础掉率再乘伤害比例、地图等级和攻击者境界差 | 挖矿等级地块伤害、工具地块伤害、环境破坏速度 | 作为矿物最终期望掉落数量的独立乘区；固定摧毁掉落不受影响 | 挖矿技艺经验倍率 |
| 营造 `building` | 当前多为建造固定成功，预留特殊建筑失败或品质判定 | 建造每息进度、营造工具速度、个人建造速度 | 返材、额外耐久、额外建筑品质、额外建筑产物 | 营造技艺经验倍率 |
| 阵法 `formation` | 布阵成功率、维护成功率，当前多为固定成功 | 布阵速度、维护速度；维护时按 `floor(maxQiOutputPerTick × (1 + speedRate))` 计算每息基础注入 | 阵法效果产出、维护灵力效率、额外范围、额外持续时间 | 阵法技艺经验倍率 |
