...
本文记录后续技艺属性重构的设计口径，不表示当前代码已经完全实现。

目标是把技艺相关的成功率、速度、产出率从分散业务逻辑、装备工具字段、等级加成、环境加成和特殊效果中收敛为统一的玩家技艺效果属性。来源可以很多，但最终消费形态应该稳定为“技艺维度 + 效果维度”，避免每新增一种加成就把业务结算逻辑再硬编码一遍。

当前阶段先作为设计记录，后续代码迁移应分批落地，并保持现有数值语义不变。

## 核心模型

技艺效果属性按技艺分类，每个技艺默认只有三类效果：

```ts
type CraftEffectKind = 'successRate' | 'speedRate' | 'outputRate';
...
严格三属性口径下，暂不新增 `costReduce`、`qualityRate`、`powerRate` 等额外效果维度。确实需要表达时，先归入三类属性：
...
| 技艺 | `successRate` | `speedRate` | `outputRate` |
| --- | --- | --- | --- |
| 炼丹 `alchemy` | 炼丹成功率、五行匹配后的额外修正、技艺等级成功率、幸运成功率、工具成功率 | 炼丹耗时修正、技艺等级速度、工具速度 | 成丹数量、额外成丹、副产物、返材 |
| 炼器 `forging` | 炼器成功率、五行匹配后的额外修正、技艺等级成功率、幸运成功率、工具成功率 | 炼器耗时修正、技艺等级速度、工具速度 | 成品数量、副产物、返材、额外产物 |
| 强化 `enhancement` | 强化目标等级基础概率后的额外成功率、强化技艺等级修正、幸运成功率、工具成功率 | 强化耗时修正、强化技艺等级速度、工具速度 | 保护、返材、省耗、额外强化收益 |
| 采集 `gather` | 当前多为固定成功，预留采集失败、品质或特殊资源判定 | 草药采集等级速度、采集工具速度、环境速度 | 额外草药、稀有草药、额外采集数量 |
| 挖矿 `mining` | 当前多为地块破坏固定成功，预留矿脉判定 | 挖矿等级地块伤害、工具地块伤害、环境破坏速度 | 矿物掉落、额外矿物、稀有矿物、幸运地块掉落 |
| 营造 `building` | 当前多为建造固定成功，预留特殊建筑失败或品质判定 | 建造每息进度、营造工具速度、个人建造速度 | 返材、额外耐久、额外建筑品质、额外建筑产物 |
| 阵法 `formation` | 布阵成功率、维护成功率，当前多为固定成功 | 布阵速度、维护速度 | 阵法效果产出、维护灵力效率、额外范围、额外持续时间 |
...
1. 新增共享类型和空快照构造：`CraftEffectStats`、`CraftEffectPatch`。
2. 在玩家属性结算中从装备、功法、buff、runtimeBonuses、环境和阵法收集技艺效果来源。
...
4. 优先迁移预览和结算最容易对齐的路径：
   ...
   - 挖矿地块伤害和掉落。
   - 采集速度和产出。
5. 迁移来源拆解 UI 和属性详情，让玩家能看到每类加成来自哪里。
6. 删除或降级旧字段前，补协议审计和最小 smoke。
...
| --- | --- | --- | --- | --- |
| 技艺工具属性 | `player.attrs.craftStats` | 炼丹、炼器、强化、挖矿、营造结算和技艺面板预估 | 不是属性面板卡片，只是隐藏投影；字段仍按旧工具名展开。 | 迁移为 `specialEffectStats.craft`，每个技艺统一为 `successRate/speedRate/outputRate`。 |
...
| 风水临时幸运 | `player.fengShuiLuck` | 并入有效幸运，最终影响掉落和稀有掉落 | 属性面板只展示合并后的幸运，不展示“风水临时幸运”来源或目标效果。 | 作为 `specialEffectSources.environment.fengShui` 来源，按规则贡献到掉落、技艺、灵气等目标属性。 |
| 灵气投影修正 | `qiProjection` | 灵气可见性、可吸收性、吸收效率 | 来源来自功法、buff、runtimeBonuses、attrBonuses，但不是属性面板的独立特殊属性卡片。 | 保留选择器能力，同时投影为可展示的 `qi.visibility`、`qi.absorbEfficiencyRate` 来源拆解。 |
| 炼体层数效果 | `specialStats.bodyTrainingLevel` / `player.bodyTraining.level` | 每层提供全属性百分比增幅 | 影响六维结算，但没有像底蕴、战斗经验一样作为特殊卡片展示。 | 归入特殊属性展示，字段可保留为 `bodyTrainingLevel`，同时在来源拆解中体现全属性增幅。 |
| 领悟进度速度 | 公式内 `progressGain` / job 投影 `progressGainPerTick` | pending 功法自悟、击杀推进、传法推进 | 当前不是玩家属性，只是公式结果或 job 投影；悟性也没有直接参与 pending 领悟速度。 | 抽成 `learning.comprehensionSpeedRate`，所有领悟加速来源先合并后再进入公式。 |
| 传法进度速度 | `transmissionSkill.level`、教师等级、学习者等级参与公式 | 传法 job 每 tick 进度 | 当前是传法公式内因子，不是面板特殊属性。 | 抽成 `learning.transmissionSpeedRate`，保留学习者、传授者、环境来源拆解。 |
| 持续资源消耗效率 | buff `sustainCost`、法宝盈能消耗等系统字段 | 持续扣 hp/qi 或法宝灵气相关消耗 | 当前由具体系统各自处理，没有统一属性。 | 如果后续出现统一减耗来源，归入 `sustain.hpCostRate`、`sustain.qiCostRate`。 |
...
其中 `qi` 不立刻替代 `qiProjection` 的选择器能力；更稳妥的迁移方式是先把隐藏效果作为属性详情和业务消费的统一快照，再逐步减少业务逻辑对工具字段、风水字段、传法公式字段和具体系统字段的直接读取。

## 不在本次设计内

- 不改变各技艺现有公式数值。
- 不把战斗 `NumericStats` 全部迁到技艺属性。
- 不把 `qiProjection` 并入技艺三属性。
- 不在 tick 热路径动态解析配置。
