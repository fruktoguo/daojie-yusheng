...
- 炼丹、炼器、强化的正向成功率修正量先加算，再进入原成功率曲线；不能直接加到最终概率上。
- 幸运每 1 点提供炼丹、炼器、强化成功率修正量 `+0.01`。
...
客户端自定义草稿允许辅料数量降到 0 后继续保留该行，便于玩家反复微调；保存本地预设和开始制造时必须过滤数量为 0 的辅料，服务端也按非正数量材料不存在处理。

如果以后要做“玩家真正发明新配方”，那是服务器持久化玩法，不属于本轮本地自定义配方。

## 材料五行预算规则

材料的 `materialValues.elements` 不能随意填写，必须先按等级、品阶和五行类型数计算总预算，再把预算分配到具体五行。目标是避免高低阶倒挂，并让多五行材料保持“总值更高、单系更弱”的定位。
...
totalElementValue = round(base * gradeMultiplier[grade] * typeMultiplier[elementTypeCount])
```

约束：

- 同品阶下，材料等级越高，`totalElementValue` 必须越高或持平，不允许低级材料总值超过明显高级材料。
- 同等级下，材料品阶越高，`totalElementValue` 必须越高。
- 五行类型数越多，`totalElementValue` 越高，但平均每系数值越低。
- 多五行材料用于泛用和调和，不应同时压过同级单系/双系专精材料。
- 负五行材料也使用同一总预算，先分配绝对值，再按配置方向写成负值。
...
分配后使用整数，四舍五入带来的余数必须补到主系或主题最强的五行上，保证各系绝对值之和等于 `totalElementValue`。
...
例如 Lv30 earth 三系偏土材料，总值约 127，可设计为：
...
例如 Lv29 earth 五系调和材料，总值约 154，可设计为：
...
- 炼丹等级 / 炼器等级：等级差修正量与工具、幸运等修正量先加算，再进入原成功率曲线。
- 丹炉 / 炼器工具。
- 装备。
- buff。
- 建筑或环境加成。
