...
- 地图实例内部的地块灵气桶保存绝对值；玩家视角同步中的 `tile.aura` 已由服务端合并可吸收资源的 `effectiveValue` 并换算为最终灵气等级，客户端必须直接展示，不能再次调用 `getAuraLevel`。
- 地块存在多类气机资源时，优先使用服务端给出的 `level`；缺少等级时才按 `effectiveValue/value` 换算，最终以等级最高的资源家族和等级着色。
- 地图感气提示和观察主信息只显示当前玩家可吸收气机合计后的最终灵气等级，不逐项罗列凝练、逸散或五行灵气。
- 观察弹层右侧气机卡片按资源分别显示 `effectiveValue` 和 `level`；二者均已按当前玩家的气机投影系数折算，不展示未折算的原始资源值。
