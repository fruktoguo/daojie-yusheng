...
- 最大人数固定等于当前空间总格数：`5×5=25`、`7×7=49`、`9×9=81`，不提供独立容量配置。在线玩家和离线保留位置使用同一准入口径。
...
「开启」面板展示密室详情、每小时运行成本、开启时长和应付总额。时长显示不可直接输入，只能通过 `÷2 / -1 / +1 / ×2` 调整，范围为 1 至 168 小时。服务端按当前 revision、倍率、空间派生容量和时长重新计算资产变更，客户端金额不作为结算依据。密室已经开启时，面板只显示统一截止时间和进入按钮，不再显示付款控件。

「管理」面板仅创建者可用，可修改名称、整数 `1x` 至 `10x` 时间倍率和空间档位；最大人数只随空间档位变化。开启期间倍率和空间锁定，名称仍可修改。系统不提供燃料储备、经营收益或提前关闭入口。
...
- 每增加一个由空间提供的位置，运行成本在线性基础上增加 80%。容量由宽高乘积派生，容量系数为 `1 + 0.8 × (width × height - 1)`，不是复利。
...
玩法状态真源为 `instance_time_chamber_state`，保存稳定实例 ID、动态模板 ID、创建者、名称、尺寸、尺寸派生容量快照、目标倍速、全室开始/截止时间、开启者、已付总额和 revision。运行时准入与计费只以尺寸派生容量为准，不信任客户端或历史容量快照。不存在个人租期或收益账本；实例自身的 `tick / tickSpeed / paused` 继续由通用实例 time checkpoint 持久化。
