...
- 非宗门地图中，玩家主动拆除建筑时，服务端严格要求 `building.ownerPlayerId === playerId`；非本人建造或缺少建造者归属的旧建筑都不可拆除，并返回明确的权限失败原因。密室内部是唯一例外：服务端按密室实例的权威 `ownerPlayerId` 允许密室创建者清理他人或旧版本遗留的无主建筑，包括半成品和已完工建筑；普通访客仍只能拆除自己的建筑。
...
- 默认权限：`view_deposit` 为无策略，任何玩家可看和可放；`withdraw` 仅创建者可拿。创建者始终放行。
