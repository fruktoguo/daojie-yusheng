...
- 「开启」先打开独立使用面板。设定为 `1x` 时无需支付或创建开启时段，任意玩家可在容量允许时直接进入；设定为 `2x` 至 `10x` 且尚未开启时，玩家选择时长并直接支付本轮全部运行成本，事务成功后整间密室进入统一开启时段，并排队让开启者进入。
...
「管理」面板仅创建者可用，可修改名称、实际最大进入人数、整数 `1x` 至 `10x` 时间倍率和空间档位。最大人数不能超过当前空间上限；缩小空间前必须先把最大人数降到目标空间上限以内。开启期间倍率、容量和空间锁定，名称仍可修改。系统不提供燃料储备、经营收益或提前关闭入口。
...
玩法状态真源为 `instance_time_chamber_state`，保存稳定实例 ID、动态模板 ID、创建者、名称、尺寸、实际配置容量、目标倍速、全室开始/截止时间、开启者、已付总额和 revision。服务端按当前尺寸校验容量上限，准入与计费使用持久化的实际配置容量，不信任客户端提交的价格或人数判断。不存在个人租期或收益账本；实例自身的 `tick / tickSpeed / paused` 继续由通用实例 time checkpoint 持久化。
