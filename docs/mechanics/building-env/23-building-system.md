...
- 所有玩家都可以在入口 1 格内看到「开启：密室名称」，创建者额外看到「管理：密室名称」。两个交互摘要都显示设定/当前时间流速和有效使用人数/最大人数。
- 「开启」先打开独立使用面板；玩家选择并支付使用时长后，服务端创建该玩家的独立使用时段并排队进入。未购买有效时段不能直接进入；有效时段内主动离开或首次传送未完成时，可从同一面板直接重新进入，无需再次付费。
- 每个有效使用时段预占一个容量名额，包括暂时离线或提前离开的玩家；同一玩家续期不重复占位。容量由创建者配置，最小为 1，且不得超过当前空间内部可站立格数量和系统上限 100。
- 玩家位置继续使用通用 occupancy 权威索引，密室不另写重叠规则。
- 密室内提供「离开密室」动作，返回外部建筑位置，不依赖成对静态传送门。
- 使用时段按现实绝对时间保存，断线和重启不暂停。时段到期后玩家自动迁回外部入口，离线位置 checkpoint 也同步修正；提前离开不退款。
- 空间档位为小型 `9×9`、中型 `15×15`、大型 `21×21`。地图无怪物、容器和资源点，内部为封闭全域安全区。
- 只有创建者可调整空间；存在有效使用时段、密室中有玩家，或内部存在建筑、掉落、妖兽等运行态对象时拒绝调整，不在线裁切或搬迁对象。

### 开启、管理与计费

「开启」面板展示密室详情、每小时收费、使用时长和应付总额。时长显示不可直接输入，只能通过 `÷2 / -1 / +1 / ×2` 调整，范围为 1 至 168 小时。服务端按当前 revision 和每小时收费重新计算资产变更，客户端金额不作为结算依据。

「管理」面板仅创建者可用，可修改名称、每小时收费、最大人数、整数 `1x` 至 `10x` 时间倍率和空间档位，并可投入运行燃料、查看/领取经营收益。存在有效使用时段时，倍率、容量和空间锁定；名称和未来订单价格仍可修改，已购买时段不补差价。

- 创建者开启自己的密室免使用费，但仍会占用名额和消耗密室运行燃料。其他玩家的使用费进入密室收益账本，创建者可在背包容量允许时领取。
- 没有任何有效使用时段时，密室实际倍率固定为 `1x` 且不消耗燃料；只要存在至少一个有效时段，就应用管理端设定倍率。
- `1x` 每小时运行成本为 0。`s >= 2` 时，单位置基础成本为 `50 × 2^(s - 2)` 灵石/小时，即 `2x=50`、`3x=100`、`4x=200`，直至 `10x=12800`。
- 每增加一个最大容量位置，运行成本在线性基础上增加 80%。容量系数为 `1 + 0.8 × (capacity - 1)`，不是复利；最终每小时成本为“倍率基础成本 × 容量系数”。
- 每枚投入灵石转换为 `36000` 个燃料单位（内容配置 `timeChamber.fuelUnitsPerSpiritStone` 可调整）。购买或续期时，只对所有有效时段并集新增的现实时间区间预扣运行燃料；多人重叠时段不会重复收取同一段运行成本。
- 投入燃料、购买时段和领取收益均使用 durable operation，在同一数据库事务内更新玩家背包、密室燃料/收益、使用时段、watermark、资产审计和 outbox；重复 operation ID 不会重复扣款或入账。
- 运行成本已经在购买时段时预扣，tick 热路径不访问数据库也不再次扣费。到期边界由实例调度器和集中到期队列共同收敛，不为每个密室建立常驻高频轮询。

玩法状态真源为 `instance_time_chamber_state`，保存稳定实例 ID、动态模板 ID、创建者、名称、尺寸、容量、目标倍速、每小时收费、数据库燃料、待提收益和 revision；`instance_time_chamber_usage` 保存每名玩家的现实开始/截止时间、成交价、已付费用和分摊的新增运行成本。实例自身的 `tick / tickSpeed / paused` 继续由通用实例 time checkpoint 持久化。

### 拆除与恢复

- 主动拆除前必须确认没有有效使用时段且密室无人；随后在同一数据库事务中锁定 `instance_catalog`，活跃 lease 必须与本地运行态的 `assignedNodeId / leaseToken / ownershipEpoch` 完全一致，销毁时递增 ownership epoch 以隔离旧 writer，再级联删除密室状态与使用时段。事务成功后才允许删除外部建筑。
- 被攻击至 0 耐久时不能绕过异步释放链，建筑会保留 1 点耐久等待正常拆除。
- 启动禁建区自检会预检将被摧毁的密室，并调用同一释放入口；释放失败的可恢复建筑加入豁免集合，避免产生孤儿实例。
- 启动先加载密室状态与使用时段并注册动态模板，再恢复实例目录与 checkpoint；建筑恢复完成后先迁出已到期玩家，再重新应用实际倍率和实例 deadline，最后才开放 tick。
