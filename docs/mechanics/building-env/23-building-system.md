...
- 非宗门地图中，建筑所有者可以立即拆除自己的建筑；其他玩家也可以拆除，但必须靠近目标持续工作，耗时严格等于该建筑建造时记录的 `buildStrength`。建筑完工后 `buildRemainingTicks` 会归零，但 `buildStrength` 继续随建筑实例持久化，因此不需要从当前配置反推历史建造耗时。离开目标、被打断或主动取消会恢复拆除前的建筑状态，已推进的拆除进度不保留。
- 宗门地图中，放置/继续建造与拆除仍分别要求当前职位拥有宗门 `building_create` / `building_remove` 权限。通过拆除权限管理其他成员建筑时同样进入上述等时拆除任务；宝库库存返还、密室释放和审计链仍按原建筑 owner 执行。
- 密室创建者不再获得对他人建筑的立即拆除特权；其清理他人或旧版本遗留无主建筑时同样按 `buildStrength` 逐息拆除。普通访客在非宗门准入规则下也可发起等时拆除。
...
- 两个槽位都可使用通用的好友关系、同宗门职位、指定玩家序号、角色名字匹配、境界和属性条件；每项最多两类条件，支持“或/且”。旧存档中的同队伍范围作为隐藏兼容条件保留。
