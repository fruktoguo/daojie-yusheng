...
- 非宗门地图中，建筑所有者可以立即拆除自己的建筑；其他玩家也可以拆除，但必须靠近目标持续工作，拆除总工作量严格等于该建筑建造时记录的 `buildStrength`，并按拆除者当前 `craftEffectStats.building.speedRate` 使用与建造相同的每息加速口径推进。建筑完工后 `buildRemainingTicks` 会归零，但 `buildStrength` 继续随建筑实例持久化，因此不需要从当前配置反推历史建造工作量。离开目标、被打断或主动取消会恢复拆除前的建筑状态，已推进的拆除进度不保留。
...
- 密室创建者不再获得对他人建筑的立即拆除特权；其清理他人或旧版本遗留无主建筑时同样以 `buildStrength` 为总工作量并应用自身建造加速。普通访客在非宗门准入规则下也可发起拆除。
