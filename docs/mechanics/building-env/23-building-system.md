...
- `spirit_farmland` 是可通行的单格地板层建筑，基础建造工时 60 息、材料 1 枚灵石，走通用营造任务生命周期与现有耐久规则。
- 只有已完工、空置且无其他同格建筑的灵田可以使用种子；一格一株，自然采集物不要求灵田。
- 种植物七天地图时间到期后只清除植物，灵田保留；灵田被拆除或摧毁则连同其植物清除。
...
- 所有玩家都可以在入口 1 格内看到「开启：密室名称」，创建者额外看到「管理：密室名称」。两个交互摘要都显示设定时间流速和当前人数/最大人数。
...
- `1x` 每小时运行成本为 0。`s >= 2` 时，单位置基础成本为 2 倍速 100 灵石/小时，之后按梯度阶梯递增（3x=150 为 2x 的 1.5 倍、4x=300 为 3x 的 2 倍、5x=750 为 4x 的 2.5 倍、6x=2250 为 5x 的 3 倍、7x=7875 为 6x 的 3.5 倍、8x=31500 为 7x 的 4 倍、9x=157500 为 8x 的 5 倍、10x=945000 为 9x 的 6 倍）。
