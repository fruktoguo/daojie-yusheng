...
- 幸运变化按签运比例浮动：基础区间内使用 `floor(比例 × 30 - 10)`，超过基础满值后使用 `floor(比例 × 20)`；签运只作为当日临时幸运修正，过了中国时区零点即失效，最终有效幸运不低于 0
- 若本次随机功德正好等于实际随机上限，则视为满签 `天命唯一`，当日固定获得 `666` 临时幸运；未满的极高值仍按比例结算
- 永恒每次使用为固定池增加 1000 功德
- 每日签到领取记录、当日签运载荷与功德背包后态共享一次 durable transaction；事务内发现当天状态或奖励快照已变化时整笔回滚。
