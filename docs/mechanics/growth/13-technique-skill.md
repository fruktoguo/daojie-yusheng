...
纯被动功法（功法内所有技能均为 `active: false` 且带 `passiveEffects`）不再使用模板的 `maxLayer` 作为修炼上限：

- 第 1 层沿用模板第 1 层的升级经验；第 `n` 层升级经验为 `baseExp × 1.4^(n-1)`，按整数经验保存，不设置玩法层数上限；仅在 JavaScript `Number` 发生数值溢出时使用 `Number.MAX_VALUE` 作为计算保护值。
- 被动效果以第 1 层配置为基准，每提升一层，效果强度增加 5%；第 21 层的强度倍率为 `2.0`，例如物理攻击 `+8%` 变为 `+16%`。
- 该倍率同时作用于被动 Buff 的属性/数值/气机投影和技艺加成，以及修炼地块注灵效果；不改变技能解锁、范围、层数上限元数据等非强度字段。
- 被动功法始终按“未圆满”处理，详情只展示当前层数、经验和被动效果，不展示有限层数星图。

### 战斗被动功法平衡

- 五行专修、武体专修与术心专修共 28 本战斗被动功法，负面代价按品阶正面核心值重算：凡阶 4%、黄阶 6%、玄阶 8%、地阶 14%。武体的最大灵力代价为上述值的一半；术心的物理防御代价为核心值的 40%，最大生命代价为核心值的 25%。
- 土系与水系 8 本防御专修额外提供仇恨获取，分别为凡阶 16%、黄阶 24%、玄阶 32%、地阶 56%。
- `extraAggroRate` 是独立的有符号百分比点属性。功法必须通过单独的 `statMode: 'flat'` 被动 Buff 直接增减该值，不能放入普通 `statMode: 'percent'` 属性 Buff，避免把百分比点再次作为数值乘区放大。
...
- 批量领悟仅开放内功。玩家指定 `1-100` 部后，每枚悟道玉简分别创建一个独立功法任务，品阶、境界和 `80%-120%` 强度各自随机；服务端固定六维等权，不让 AI 生成属性、权重、层数或技能。AI 单次只返回对应数量且顺序一致的名称与描述 JSON，服务端校验数量、字段与字数后，若同批重名或与已发布功法重名则自动追加序号去重，再整批生成草稿。采纳时若遇重名同样自动追加序号，确保批量领悟始终可正常学习。
- 批量任务的玉简扣除、子任务创建、失败返还、整批采纳和整批放弃均使用玩家会话栅栏与持久化事务。草稿必须整批采纳或整批放弃；采纳时全部功法分别写入待领悟真源，不会直接视为已经学会。服务重启后按批次恢复未完成任务，禁止只恢复或提交其中一部分。
- 推演任务进入 `pending/running` 后前一分钟不可手动取消；超过 1 分钟仍未生成草稿时，玩家可在功法领悟界面取消推演，服务端把任务标为 `cancelled` 并原路返还已扣除的悟道玉简。超过 30 分钟仍未生成完整草稿的 `pending/running` 任务会自动取消并返还悟道玉简；后台清理与玩家重新打开状态面板都必须确保该任务不再占用 active job。
...
自动切换候选必须优先选择仍可修炼的有限层数功法与允许自悟的 pending 功法；只有这些有限目标全部达到当前上限或完成领悟后，才允许切换到纯被动无限层数功法。

设置或取消主修只修改后续修炼目标，不等同于开启或停止闭关修炼。取消主修后若仍保持修炼，境界修为照常推进，原本分配给功法的经验全量转入炼体；取消前主修是有限层数或无限层数功法均使用相同规则。炼丹、炼器、强化等技艺 job 进行中仍可切换主修；真正开启修炼由独立修炼开关控制，并按通用打断规则处理当前技艺 job。
...
`self_comprehension_allowed` 表示是否允许通过主修修炼自行领悟，是单向粘性标记：默认 `false`，只能被升级为 `true`，任何路径不得把已有 `true` 改回 `false`。功法书开启的普通功法、他人抄录的自创功法（含残卷）、自己创建的自创功法均为 `true`，学书不要求学习者是作者；自创功法仍按自创领悟需求计算，残卷仍受可修层数上限约束。被传法或藏经参悟 job 新建的 pending 功法为 `false`，只能由 job 推进，不能设为主修；客户端按钮必须置灰，服务端必须拒绝该主修切换。传法/藏经参悟 job 的启动、tick、取消均不得修改已有 pending 的该字段——job 接管期间的自修互斥由 `transmissionJob`/`activeTransferJob` 锁保证，而非该布尔值；job 取消或完成后，原本为 `true` 的条目仍可自行领悟。已有未领悟条目再使用对应功法书时，保留领悟进度并开放自行领悟。
