...
每座统法台只能承载一个法脉。首次凝篇至少选择两本源功法，并由台主设定 2-20 字的法脉名讳；凝篇成功后，建筑永久绑定该稳定 `familyId`，法脉名讳和所录品阶随之固定，不得换绑其他法脉。后续以递增 `revision` 发布不可变新卷，必须在最新版全部叶子集合上至少新增一本，不允许删减或改写旧版。功法书、传法、统法台参悟和藏经台若指向旧版，学习时统一解析为该法脉最新版，并重新按当前覆盖量走领悟流程。
...
统法台面板按品阶和境界筛选候选法卷，以固定卡格分页展示；每页最多 12 部，并提供当前筛选范围的全选和全部取消。候选卡显示服务端权威生成强度 `80%-120%`、境界、层数与圆满状态；这些字段只用于低频面板展示，客户端不据此重算功法效果。
