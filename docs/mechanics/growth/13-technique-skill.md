...
功法统合是统法台的低频交互；统法台属于不阻挡移动和视线的设施建筑，玩家必须位于已完工统法台 1 格内，所有候选、权限、版本、门规和重叠校验均由服务端权威执行。当前只允许统合同品阶的自创内功，源功法必须由台主本人创建并修至模板真实满层；系统功法、术法、神通、秘法、残卷上限或仅 `expToNext = 0` 都不能绕过校验。
...
台主可以为已绑定法脉刻录传法门规。门规可设为完全不限制，也可多选好友层级 `dao_friend`（道友以上）/`close_friend`（至交），以及同宗门的宗主、太上长老、副宗主、长老、内门弟子、外门弟子、杂役弟子等精确职位；限制项按“满足任意一项”放行，台主本人始终可用。好友关系、台主当前宗门、学习者当前宗门和职位在每次打开面板与提交参悟时实时读取，关系或职位变动立即生效。未选择任何限制项且未开启完全不限制时，仅台主可阅。
...
统合元数据作为生成功法模板 JSON 与不可变版本一起持久化，并冗余记录首次凝篇所在实例、建筑与初始门规，用于模板发布成功但建筑域尚未刷盘时恢复绑定。统法台当前绑定的 `familyId` 和可变门规写入通用 `instance_building_state.payload`，不另建重复真源；玩家仅按现有 `player_technique_state` 和 `player_technique_comprehension` 保存当前统合功法 ID 及动态进度。服务器启动时先恢复全部已发布统合模板，再水合建筑与玩家态；运行期领悟和重叠检查只读内存索引，不在 tick 热路径访问数据库。
