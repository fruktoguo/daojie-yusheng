...
功法统合是炼法台的低频交互；玩家必须位于已完工炼法台 1 格内，所有候选、权限、版本和重叠校验均由服务端权威执行。当前只允许统合同品阶的自创内功，源功法必须由操作者本人创建并修至模板真实满层；系统功法、术法、神通、秘法、残卷上限或仅 `expToNext = 0` 都不能绕过校验。

首次统合至少选择两本源功法。统合功法按稳定 `familyId` 和递增 `revision` 发布不可变版本；后续更新必须在最新版全部叶子集合上至少新增一本，不允许删减或改写旧版。功法书、传法和藏经台若指向旧版，学习时统一解析为该家族最新版，并重新按当前覆盖量走领悟流程。

统合模板不封存、不删除源功法；其他已有源功法的玩家仍可继续修炼、传授和抄录。玩家领悟统合功法完成时，只从该玩家态中移除被覆盖的直接源功法、同家族旧版及对应未领悟进度，保留新版一行。发布者发布成功后直接获得满层新版，并执行同样的个人态替换。

领悟成本按源功法叶子数量计算，不按玩家功法表的行数计算。已学直接源功法、已学统合版本和其他待领悟统合版本都提供叶子覆盖，剩余需求为基准领悟需求乘以未覆盖叶子比例，最低保留 1 点。候选统合功法自身的 pending 行不计入覆盖，避免尚未领悟就被判定为 100% 已掌握。
...
统合元数据作为生成功法模板 JSON 与不可变版本一起持久化；玩家仅按现有 `player_technique_state` 和 `player_technique_comprehension` 保存当前统合功法 ID 及动态进度。服务器启动时先恢复全部已发布统合模板，再水合玩家态；运行期领悟和重叠检查只读内存索引，不在 tick 热路径访问数据库。
