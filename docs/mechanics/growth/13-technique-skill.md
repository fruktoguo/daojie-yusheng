...
功能尚未正式开放期间，统法台固定消耗 1 份无常规产出且不可进入交易行的「测试材料」建造，以阻止正式玩家提前创建。开放该玩法时必须先替换为正式建造配方，并为正式材料配置可审计的获取来源。

每座统法台只能承载一个法脉。首次凝篇至少选择两本源功法，并由台主设定 2-20 字的法脉名讳；凝篇成功后，建筑永久绑定该稳定 `familyId`，法脉名讳和所录品阶随之固定，不得换绑其他法脉。后续以递增 `revision` 发布不可变新卷：录入自有功法时必须在最新版全部叶子集合上至少新增一本，不允许删减或改写旧版；融入悟道玉简时保持叶子集合不变，只追加法脉自身的均衡道韵。功法书、传法、统法台参悟和藏经台若指向旧版，学习时统一解析为该法脉最新版，并重新按当前覆盖量走领悟流程。

统法台分别保存“参阅权限”和“修订权限”，两者互不隐含。参阅权限决定谁可将台上法脉纳入参悟；修订权限决定谁可向已绑定法脉续录自己的功法。两项权限都可设为完全不限制，也可多选好友层级 `dao_friend`（道友以上）/`close_friend`（至交），以及同宗门的宗主、太上长老、副宗主、长老、内门弟子、外门弟子、杂役弟子等精确职位；限制项按“满足任意一项”放行，台主本人始终可用。默认参阅不限制，默认修订仅限台主；未选择任何限制项且未开启完全不限制时，仅台主可用。

好友关系、台主当前宗门、来访者当前宗门和职位在每次打开面板与提交操作时实时读取，关系或职位变动立即生效。修订者只能提交由自己创建、已修至模板真实满层且与法脉同品阶的自创内功；既有叶子保留原创建者归属，不要求修订者持有或重修。法脉初创者保持为台主，每卷另记实际修订者用于审计和幂等；修订成功后，修订者直接获得最新版并按同一规则替换其个人功法态。

统合模板不封存、不删除源功法；其他已有源功法的玩家仍可继续修炼、传授和抄录。玩家领悟统合功法完成时，只从该玩家态中移除被覆盖的直接源功法、同家族旧版及对应未领悟进度，保留新版一行。发布者发布成功后直接获得满层新版，并执行同样的个人态替换。
...
已绑定法脉且拥有修订权限的玩家，可选择消耗 `1-100` 枚悟道玉简直接录法，实际单次上限还受当前背包持有量约束。该路径不调用 AI，不创建独立源功法，也不占用新的源功法 ID；服务端沿用法脉既定品阶和当前最高境界，对每枚玉简分别随机一次 `80%-120%` 强度预算，并将各次所得基础六维平均分配为五行无偏的均衡道韵，最后合并凝成同一不可变新卷。道韵与源功法属性一同享受统合 `110%` 效果，可以多次累计，但不增加 `sourceCount`、叶子覆盖、领悟折减分母或总修炼难度。尚未立脉的统法台不能直接以玉简凝成首卷。

每次玉简录法都必须携带幂等操作号，所选数量同时进入请求指纹。扣除指定数量的悟道玉简、插入新不可变统合卷、耐久操作日志、资产审计和背包 outbox 在同一玩家事务与会话栅栏内提交；同一操作重放只回读既有卷次和已提交背包快照，不得再次扣除或重掷强度，复用操作号但改变数量必须按载荷冲突拒绝。物品不足、权限失败、数量越界、卷次冲突或持久化失败时均不得产生半成品卷次。

统法台面板使用“总览 / 录法 / 权限”主 Tab；录法仅对拥有修订权限者显示，权限仅对建造者显示。总览展示最新版完整源法名录、累计玉简道韵数量，以及服务端按法卷真实层数据计算的满层六维总加成，客户端不得自行重算。录法页再分为“录入自有功法”和“融入悟道玉简”。自有功法目录按品阶和境界筛选，以固定卡格分页展示，每页最多 12 部，并提供当前筛选范围的全选和全部取消；候选卡左下角显示服务端权威生成强度 `80%-120%`，是否选中只通过边框反馈，同时保留境界、层数与圆满状态。玉简录法先由玩家指定本次数量并显示消耗、录后累计与背包余量预检，再进入二次确认；预检不得提前揭示随机强度。手机端源法目录和总览名录均随正文纵向展开，不建立嵌套滚动区；这些字段只用于低频面板展示，客户端不据此重算功法效果。

统合元数据作为生成功法模板 JSON 与不可变版本一起持久化，并冗余记录首次凝篇所在实例、建筑与初始参阅/修订权限，用于模板发布成功但建筑域尚未刷盘时恢复绑定。统法台当前绑定的 `familyId` 和两项可变权限写入通用 `instance_building_state.payload`，不另建重复真源；玩家仅按现有 `player_technique_state` 和 `player_technique_comprehension` 保存当前统合功法 ID 及动态进度。服务器启动时先恢复全部已发布统合模板，再水合建筑与玩家态；运行期领悟和重叠检查只读内存索引，不在 tick 热路径访问数据库。
