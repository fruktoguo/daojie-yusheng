...
已绑定法脉且拥有修订权限的玩家，可选择消耗一枚悟道玉简直接录法。该路径不调用 AI，不创建独立源功法，也不占用新的源功法 ID；服务端沿用法脉既定品阶和当前最高境界，只随机一次 `80%-120%` 强度预算，并将所得基础六维平均分配为五行无偏的均衡道韵。道韵与源功法属性一同享受统合 `110%` 效果，可以多次累计，但不增加 `sourceCount`、叶子覆盖、领悟折减分母或总修炼难度。尚未立脉的统法台不能直接以玉简凝成首卷。

每次玉简录法都必须携带幂等操作号。扣除一枚悟道玉简、插入新不可变统合卷、耐久操作日志、资产审计和背包 outbox 在同一玩家事务与会话栅栏内提交；同一操作重放只回读既有卷次和已提交背包快照，不得再次扣除或重掷强度。物品不足、权限失败、卷次冲突或持久化失败时均不得产生半成品卷次。

统法台面板使用“总览 / 录法 / 权限”主 Tab；录法仅对拥有修订权限者显示，权限仅对建造者显示。录法页再分为“录入自有功法”和“融入悟道玉简”。自有功法目录按品阶和境界筛选，以固定卡格分页展示，每页最多 12 部，并提供当前筛选范围的全选和全部取消；候选卡左下角显示服务端权威生成强度 `80%-120%`，是否选中只通过边框反馈，同时保留境界、层数与圆满状态。手机端目录随正文纵向展开，不建立嵌套滚动区；这些字段只用于低频面板展示，客户端不据此重算功法效果。
