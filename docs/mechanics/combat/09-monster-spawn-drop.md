...
副本房间可在 `bossDropTable` 中声明 Boss 专用掉落表。运行时将该表挂到副本 Boss 实例上，击杀和掉落预览均优先使用它；普通地图仍读取 `monsterDropsByMonsterId`，因此不会被副本奖励覆盖。

副本专用表中的灵石、功德使用固定基础数量并保证掉落，随后按副本难度数量倍率取整：试炼 `1.0`、困难 `1.5`、噩梦 `2.5`、现世 `4.0 × (1 + 0.2 × 现世阶位序号)`。功法掉率则把等效击杀次数乘以难度倍率：试炼 `1`、困难 `2`、噩梦 `3`、现世 `5 × (1 + 0.5 × 现世阶位序号)`，再叠加击杀者自身的掉率属性。副本激活快照每多 1 名队员，功法等效掉率再乘 `1 + 0.5 × (人数 - 1)`（1~5 人为 1.0/1.5/2.0/2.5/3.0），仍只掷一次，不复制掉落，也不改变货币数量。
...
矿物地块不再使用固定 `100` 伤害门槛。灵石矿、玄铁矿等带 `miningLevel` 标记的结构统一进入地图等级矿物公式：以地块 `0.1% maxHp` 为伤害基准，低于基准按实际伤害比例线性缩放，超过基准按平方根平滑增长（如 10% 伤害增幅由原本 100 倍平滑为 10 倍）；地图 `mapLv` 结合 10% 指数复利与降低 10 倍后的分段线性增幅（1-10 级每级 +10%，11-20 级每级 +20%...相加叠加）；之后再乘攻击者境界等级差、同一玩家同次攻击多矿 AOE 衰减和挖矿其他增幅，并将最终期望数量转换为最高 10% 的触发率与随机数量。完整公式见 `docs/mechanics/core-loop/04-map-terrain.md`。
...
- 怪物掉落仅对实际击杀者生效：掉落链路的等效击杀次数（`killEquivalent`，含玩家自身 `lootRate`/`rareLootRate` 加成）翻倍，稀有掉落同样受翻倍影响。
- 增益只进入怪物击杀经验与怪物掉落链路，不影响修炼、任务、采集、地块、容器、PVP 或其他经验和掉落来源。

### 队伍奖励分配

- 队伍贡献模式沿用全体参战者原始伤害贡献比例。
- 队伍平均模式先固定该队在全体贡献中的总份额，再仅在同实例、存活、20 格内且有伤害或有效支援记录的成员间均分；不会复制整份经验。
- 平均模式的合格成员统一进入现有击杀进度入口，击杀任务按同一资格推进；跨图、死亡或无参与成员不获得进度。
- 掉落始终使用实际击杀者属性且只掷一次。`killer` 模式交付击杀者；`round_robin` 模式在同一资格边界内逐堆栈稳定轮转。公开世界不因人数增产；副本另有组队爆率，见上节，仍不按人复制。
