...
副本专用表中的灵石、功德使用固定基础数量并保证掉落，随后按副本难度数量倍率取整：试炼 `1.0`、困难 `1.5`、噩梦 `2.5`、现世 `4.0 × (1 + 0.2 × 现世阶位序号)`。功法掉率则把等效击杀次数乘以难度倍率：试炼 `1`、困难 `2`、噩梦 `3`、现世 `5 × (1 + 0.5 × 现世阶位序号)`，再叠加击杀者自身的掉率属性。
...
- 掉落始终使用实际击杀者属性且只掷一次。`killer` 模式交付击杀者；`round_robin` 模式在同一资格边界内逐堆栈稳定轮转，不扩大总产出。
