...
副本例外：`engaged=false` 的副本怪物不执行主动寻敌，也不因玩家进入视野积累被动仇恨；首次受到玩家攻击后切换为 `engaged=true`，再按副本全图追击规则行动。
...
- `100%~75%`：优先断魂灵钉；不可用时继续进入通用战术决策，最终才由残魄掌兜底。
- `75%~50%`：近中距离优先移脉熔宫，远距离使用断魂灵钉；不要求残丹法相 Buff。
- `50%~25%`：优先星罗残盘、熔河贯脉；不要求残丹法相 Buff。
- `25%及以下`：若尚无法相则优先释放残丹法相虚影；法相不可用时仍可释放地府沉印、裂府外环、锁宫内环。
- 法相属于绝境强化状态，只改变自身属性、表现尺寸和技能范围，不是其他技能的施放前置条件。
