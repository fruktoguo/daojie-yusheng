...
全局 dispatcher 两次帧开始之间至少间隔 `100ms`，对应当前实例最高 `10x`，不会因为大量错峰 deadline 退化为 `5ms` 轮询。`setTimeout` 的单次剩余等待量仍可能小于 `100ms`（例如本帧已执行 `95ms` 时只需再等 `5ms`），但它不是逻辑 tick 周期。慢帧指标直接读取 deadline 调度器因补帧上限真实丢弃的逻辑息，不再根据 timer 剩余等待量估算；丢弃累计值每帧回填到 `world-tick` 调度任务的 `backlogCount`，在 GM worker 面板的 Scheduler 任务列表中以"积压丢弃"展示，不再产生日志告警。
