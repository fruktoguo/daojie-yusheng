...
当前内置来源“巡天飞剑”在对应法宝槽已解锁、启用、装备且法宝灵力大于 0 时，为玩家提供忽略静态障碍能力。移动裁定只检查玩家是否拥有该能力，不在移动时读取物品或扣除法宝灵力；法宝灵力消耗由玩家 tick 的法宝运行时统一持续扣除。穿越不可移动静态地块时，仍按该地块原始 `TILE_TRAVERSAL_COST` 扣除移动点数。
