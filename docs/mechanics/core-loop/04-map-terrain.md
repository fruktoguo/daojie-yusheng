...
天然矿物等级取所在地图的 `mapLv`，晶精矿取生成时保存的等级。每个矿脉目标按自己的实际伤害和最大生命独立计算，但不再按命中次数提供固定基础掷骰收益。
...
- 灵石矿独立掉落 `spirit_vein_crystal`（灵脉晶精），玄铁矿独立掉落 `black_iron_vein_crystal`（玄铁矿脉晶精）；其他可破坏地块不掉晶精。
- 每个矿脉目标按实际扣血计算一次：`概率 = getMiningDamageDropMultiplier(实际伤害, maxHp) × 0.001 / 10000 × (1 + 幸运加成)`。伤害低于最大生命 0.1% 时线性缩放，高于阈值时按平方根缩放。单次造成 10% 伤害时，无幸运概率为 `0.0001%`（百万分之一）。
- 该独立概率只接受幸运，不接受挖矿等级、地图等级、境界差、AOE 衰减、产出率或矿脉诅咒等其他乘区。晶精每次触发掉落 1 枚。
- 使用一枚晶精在脚下生成对应矿脉，复用建造的地图准入、宗门权限及禁建范围。脚下须为空地，不能覆盖建筑或已有临时地块；允许使用者站在生成位置。
- 生成等级取地图等级与使用者境界等级较大值，耐久、普通矿物掉落及挖矿经验按该等级结算。
- 人工矿没有自然回血或自带固脉，外部固脉仍可减伤和回血；挖尽后直接移除、不复生。最多存续 `86400` 实例息，固脉不能阻止到期，10 倍密室对应现实 2.4 小时。
- 使用时背包扣除和临时地块快照在同一 durable 事务提交，校验玩家 session 与实例 lease，记录审计与 flush 水位。`instance_temporary_tile_state.source_item_id/mineral_level` 保存来源与等级，已有 `expires_at_tick` 保存地图时间截止点。

## 采集种子与植物生长

- 自然 `variant=herb` 采集节点在成功采得一个单位后，独立以 `0.2% × (1 + 幸运加成)` 掉落一枚对应种子。灵田种植物不产种子（按容器 `plantedExpiresAtTick` 标记判定），防止种子自增殖无限循环。失败、中断、攻击破坏和普通翻找容器不抽种子；采集产出率保留库存也只抽一次，不提高该概率。
- 资源节点的 `seedItemId` 关联种子模板，启动期校验并建立索引；目前覆盖全部 29 种采集节点，包括采用采集机制的异材。
- 种子仅能在脚下已建成且空置的灵田使用，每格一株，遵守建造地图准入、宗门权限、NPC/传送点等禁建规则。使用时种子消耗与 `container_state` 中的种植物、初始库存和恢复进度同事务保存，并记录实例 lease/session 围栏、审计和 flush 水位。
- 种植物与自然节点使用同一份采集物配置与生长机制，采尽后仍会补充可采数量。种植物寿命固定为 `604800` 实例息，即七天地图时间；10 倍密室实际为 16.8 小时。灵气与固脉均不能延寿，到期移除植物和剩余采集库存，保留灵田。灵田被拆除或摧毁时，依附植物也会清除。
- 自然与人工草药都按所在格普通灵气 `aura.refined.neutral` 的等级获得 `1.1^等级` 恢复速度：0 级原速，1 级 1.1 倍，10 级约 2.594 倍。该倍率加速原配置生成间隔，不改变每轮生成数量、采集用时或单次种子概率，也不消耗灵气。
- 生长保存基础剩余工作量及小数进度；灵气变动只改变后续速度，不重置进度。单息可完成多轮恢复，补算预算内未完成的工作量留待后续推进，不因最低一息取整丢失指数增幅。没有新进度结算或倍率变化时不重复标脏。
- 人工植物的来源、位置、依附灵田与截止实例 tick 保存在现有容器 metadata；库存、采集占用仍使用现有容器条目和计时表。容器 `source_id` 扩为 220 字符以容纳实例与容器的组合身份，初始化按需扩列，旧值不变。恢复时只重建实例私有索引，不修改共享地图模板，不新增高频协议字段。
