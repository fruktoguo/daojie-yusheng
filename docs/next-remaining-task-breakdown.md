...
- 这份文档保留的是旧阶段的细项拆解，不再作为当前主任务系统。
- 当前请优先看 [docs/next-plan/main.md](./next-plan/main.md) 和 `docs/next-plan/01-10`。
- 文中旧的环境阻塞、验证结果时间戳、`client-next / shared-next / server-next` 命名和 `Txx` 编号，阅读时都应视为历史快照。
