...
- [x] api 角色在权威变更发生后写入 staging，不把完整运行态对象交给 worker。
  ...
  - 边界结论：`mail` 由 `MailRuntimeService` -> `MailPersistenceService.saveMailboxMutation()`/`saveMailbox()` 直写结构化真源；`gm_edit` 由 `NativeGmPlayerService.savePlayerPersistenceSnapshotForGmUpdate()`/`savePlayerPersistenceSnapshotDomains()` 精确写入玩家分域投影并记录 GM audit；二者不是 flush staging payload domain，worker 侧保持 unsupported player domain retry 边界。
