...
- `AlchemyStrategy`、`ForgingStrategy` 的 start、tick、cancel 生命周期已拆入 pipeline strategy：start 由 strategy 提供 `validateStart`、`queueStart`、`consumeResources`、`createJob`，tick 由 `alchemy-like-tick.helpers.ts` 承载 phase 推进和批次结算，cancel 由 `alchemy-like-cancel.helpers.ts` 承载退料、退灵石、掉地和清理 active job；interrupt 仍委托 `CraftPanelRuntimeService`。`EnhancementStrategy` 仍通过 `executeStart` / `executeTick` / `executeCancel` 全权委托回旧 service。
...
| 强化 | `C2S.StartEnhancement` -> `WorldRuntimeEnhancementService` -> `startEnhancement` | `WorldRuntimeCraftTickService` -> `tickTechniqueActivity('enhancement')` -> strategy 委托 `tickEnhancement`；取消走 `executeEnhancementCancel` | `player.enhancementJob` + `inventory.lockedItems` + `enhancementRecords` | cancel 已拆入 pipeline strategy helper；start/tick/interrupt 仍委托旧 service；资产链复杂；打断等待已从实际工作进度拆出 |
