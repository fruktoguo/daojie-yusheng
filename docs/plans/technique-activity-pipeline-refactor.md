...
- 采集、建造、阵法维护属于条件型技艺；采集/建造/阵法维护的 sleeping 重试已经能通过 strategy 条件检查和 pipeline start 恢复，tick 条件暂时失败也能进入统一 sleeping 队列；采集/建造结算和外部占用推进已从 tick 编排层进入 pipeline，但真实领域规则仍委托旧 runtime 服务。阵法维护 start/cancel/tick 规则已迁入 `FormationStrategy` 和 strategy helper，旧 formation service 只保留阵法查询、状态读写、条件检查和持久化 helper。
...
| 采集 | `StartGather` -> `GatherStrategy.executeStart` -> `WorldRuntimeLootContainerService.dispatchStartGather` | `GatherStrategy.executeTick` 委托 `tickGather`；完成授予、经验、容器剩余和 activeSearch 仍在 loot container service | active job、inventory、profession、container_state；采集完成只改运行态背包并标记 dirty domain，禁止在 tick 内调用 durable grant / presence fence | 统一任务列表可见 running/sleeping/cancel；掉落/容器面板不再是唯一入口 |
...
- 2026-05-27：采集 start/cancel 从旧委托迁入标准 pipeline lifecycle：`GatherStrategy` 不再实现 `executeStart` / `executeCancel`，而是通过 `validateStart`、`consumeResources`、`createJob`、`startDirtyDomains`、`computeRefund` 和条件型 `onConditionFailed` 承载启动校验、调用真实采集 runtime 注册 activeSearch、读取 created job、标记 active_job、取消时释放 activeSearch。`world-runtime-craft-smoke` 增加采集 lifecycle proof，覆盖 sleeping 恢复通过 pipeline start 创建 `gatherJob`、取消通过公共 cancel 释放 activeSearch 并标记 active_job dirty；`world-runtime-loot-container-smoke` 继续证明采集完成 tick 不调用 durable grant / presence fence。`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/world-runtime-loot-container-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`node packages/server/dist/tools/technique-activity-cancel-ref-smoke.js` 通过。采集 tick 仍委托 `WorldRuntimeLootContainerService.tickGather`，所以不能据此勾选“旧 service 不再承载技艺玩法规则”。
