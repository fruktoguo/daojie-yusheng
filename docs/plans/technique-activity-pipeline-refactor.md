...
| 强化 | `C2S.StartEnhancement` -> `WorldRuntimeEnhancementService` -> `startTechniqueActivity('enhancement')` -> `EnhancementStrategy.validateStart/queueStart/consumeResources/createJob`；兼容 `startEnhancement` 只作为 facade 回到统一入口 | `WorldRuntimeCraftTickService` -> `tickTechniqueActivity('enhancement')` -> `EnhancementStrategy.executeTick` -> `executeEnhancementTick`；中断走 `executeEnhancementInterrupt`；取消走 `executeEnhancementCancel` | `player.enhancementJob` + `inventory.lockedItems` + `enhancementRecords` | start/tick/interrupt/cancel 已拆入 pipeline strategy 生命周期；finish/advance 部分 hook 仍委托旧 service；资产链复杂；打断等待已从实际工作进度拆出 |
...
- [x] 强化取消走 pipeline strategy helper，释放锁定装备、清 active job 和强化记录状态仍复用权威 `finishEnhancementJob`。
