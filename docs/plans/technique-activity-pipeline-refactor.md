...
- 2026-05-27：炼丹/炼器 pipeline 通知改为结构化 `key/vars/pills`：启动、入队、打断等待、取消、批次成功/失败、完成、队列续跑/跳过都不再在该链路拼玩家可见中文；新增 `notice.craft.alchemy.*`、`notice.craft.queue.*`、`notice.craft.activity-interrupted-wait` i18n 源和生成产物，`world-runtime-alchemy-smoke` 改为断言 structured key。`pnpm --filter @mud/server compile`、`pnpm --filter @mud/client exec tsc --noEmit --pretty false`、`node packages/server/dist/tools/world-runtime-alchemy-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/technique-activity-cancel-ref-smoke.js`、`pnpm audit:protocol`、`pnpm verify:client`、`pnpm verify:quick` 通过；`verify:quick` 中 session reaper 的 `simulated_flush_failure` 是用例内故障注入且最终通过。强化和其他旧 runtime service 文本 notice 仍待后续迁移。
