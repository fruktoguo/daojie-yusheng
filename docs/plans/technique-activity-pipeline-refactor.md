...
- `AlchemyStrategy`、`ForgingStrategy` 的 start、tick、cancel 生命周期已拆入 pipeline strategy：start 由 strategy 提供 `validateStart`、`queueStart`、`consumeResources`、`createJob`，tick 由 `alchemy-like-tick.helpers.ts` 承载 phase 推进和批次结算，cancel 由 `alchemy-like-cancel.helpers.ts` 承载退料、退灵石、掉地和清理 active job；interrupt 仍委托 `CraftPanelRuntimeService`。`EnhancementStrategy` 的 start、tick、cancel 已接入 strategy 生命周期；interrupt、finish/advance 的部分权威 hook 仍委托旧 service。
...
| 强化 | `C2S.StartEnhancement` -> `WorldRuntimeEnhancementService` -> `startTechniqueActivity('enhancement')` -> `EnhancementStrategy.validateStart/queueStart/consumeResources/createJob`；兼容 `startEnhancement` 只作为 facade 回到统一入口 | `WorldRuntimeCraftTickService` -> `tickTechniqueActivity('enhancement')` -> `EnhancementStrategy.executeTick` -> `executeEnhancementTick`；取消走 `executeEnhancementCancel` | `player.enhancementJob` + `inventory.lockedItems` + `enhancementRecords` | start/tick/cancel 已拆入 pipeline strategy 生命周期；interrupt 和 finish/advance 部分 hook 仍委托旧 service；资产链复杂；打断等待已从实际工作进度拆出 |
