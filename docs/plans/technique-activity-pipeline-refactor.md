...
- 任务列表里的每个可取消项都有取消按钮，不要求进入对应技艺子面板。
...
- [x] 当前任务完成后能启动下一项队列任务。
- [ ] 条件型 sleeping 项不会每 tick 热检查。
- [ ] active job 和队列都能持久化恢复。
...
- [ ] 从 `CraftPanelRuntimeService.startAlchemy` 拆出 `validateStart`、`consumeResources`、`createJob`。
...
- [ ] 只剩一种活动队列。
- [ ] active job 持久化和恢复按统一 job kind 工作。
