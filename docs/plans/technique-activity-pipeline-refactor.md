...
- [ ] 找出所有攻击、移动、手动开始修炼、切换状态等会中断技艺的入口，确认它们只写 `interruptWaitRemainingTicks` / `interruptState`，不污染实际工作进度。（玩家主动入口已审计并补 proof；跨实例传送、GM 强迁/重定位已接入统一技艺中断器并补 proof；死亡、离线/重连恢复等非主动状态切换仍待审计。）
