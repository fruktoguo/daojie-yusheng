...
- `AlchemyStrategy`、`ForgingStrategy` 的 start、tick、interrupt、cancel 生命周期已拆入 pipeline strategy：start 由 strategy 提供 `validateStart`、`queueStart`、`consumeResources`、`createJob`，tick 由 `alchemy-like-tick.helpers.ts` 承载 phase 推进和批次结算，interrupt 不再由 strategy 委托，统一走 `TechniqueActivityPipelineService.interrupt` 公共生命周期刷新独立等待条，cancel 由 `alchemy-like-cancel.helpers.ts` 承载退料、退灵石、掉地和清理 active job。`EnhancementStrategy` 的 start、tick、cancel 已接入 strategy 生命周期，interrupt 同样走公共 pipeline interrupt lifecycle；finish/advance 的部分权威 hook 仍委托旧 service。
- `WorldRuntimeCraftTickService` 已按 active kind 统一调用 `tickTechniqueActivity`；炼丹、炼器、强化、采集、建造、挖矿、阵法维护的 tick 编排都进入 pipeline。采集 tick 规则已迁入 `GatherStrategy` helper；建造 tick 规则已迁入 `BuildingStrategy` helper，旧 building runtime 的 `tickBuildingConstruction` 只保留兼容 facade。
- 制造型新 start 已写入 `player.techniqueActivityQueue`；旧 active job 内 `queuedJobs` 会在玩家水合期迁移到统一队列并清掉旧字段，少量只读 fallback 仅用于兼容尚未水合的旧形态。
- 采集、建造、阵法维护属于条件型技艺；采集/建造/阵法维护的 sleeping 重试已经能通过 strategy 条件检查和 pipeline start 恢复，tick 条件暂时失败也能进入统一 sleeping 队列。采集 tick 推进、完成、经验、容器 activeSearch 推进和 dirty 已迁入 `GatherStrategy` helper；建造 tick 推进、完工激活、经验、activeBuilder 清理和 dirty 已迁入 `BuildingStrategy` helper，实例 tick 不再自动施工。阵法维护 start/cancel/tick 规则已迁入 `FormationStrategy` 和 strategy helper，旧 formation service 只保留阵法查询、状态读写、条件检查和持久化 helper。
...
|------|----------|------------------|------------------|--------------|
| 炼丹 | `C2S.StartAlchemy` -> `WorldRuntimeAlchemyService` -> `TechniqueActivityPipelineService.start('alchemy')` -> `AlchemyStrategy.validateStart/queueStart/consumeResources/createJob` | `WorldRuntimeCraftTickService` -> `tickTechniqueActivity('alchemy')` -> `AlchemyStrategy.executeTick` -> `executeAlchemyLikeTick`；中断走 `executeAlchemyLikeInterrupt`；取消走 `executeAlchemyLikeCancel` | `player.alchemyJob`，含 `brewing`、旧存档兼容 `queuedJobs` | start/tick/interrupt/cancel 已进 pipeline strategy；经验、产出和配方型 notice 已从统一 result/helper 流出 |
| 炼器 | `C2S.StartAlchemy` + `kind=forging` -> `TechniqueActivityPipelineService.start('forging')` -> `ForgingStrategy.validateStart/queueStart/consumeResources/createJob` | `WorldRuntimeCraftTickService` -> `tickTechniqueActivity('forging')` -> `ForgingStrategy.executeTick` -> `executeAlchemyLikeTick`；中断走 `executeAlchemyLikeInterrupt`；取消走 `executeAlchemyLikeCancel` | `player.forgingJob`，历史兼容 `alchemyJob.jobType='forging'` | start/tick/interrupt/cancel 已进 pipeline strategy；经验、产出和配方型 notice 已从统一 result/helper 流出 |
| 强化 | `C2S.StartEnhancement` -> `WorldRuntimeEnhancementService` -> `startTechniqueActivity('enhancement')` -> `EnhancementStrategy.validateStart/queueStart/consumeResources/createJob`；兼容 `startEnhancement` 只作为 facade 回到统一入口 | `WorldRuntimeCraftTickService` -> `tickTechniqueActivity('enhancement')` -> `EnhancementStrategy.executeTick` -> `executeEnhancementTick`；中断走 `executeEnhancementInterrupt`；取消走公共 `cancelLifecycle` -> `EnhancementStrategy.computeRefund` | `player.enhancementJob` + `inventory.lockedItems` + `enhancementRecords` | start/tick/interrupt/cancel 已拆入 pipeline strategy 生命周期；取消不再暴露 `executeCancel` 完整委托，finish/advance 部分 hook 仍委托旧 service；资产链复杂；打断等待已从实际工作进度拆出 |
| 采集 | `C2S.StartGather` -> `TechniqueActivityPipelineService.start('gather')` -> `dispatchStartGather` | `WorldRuntimeCraftTickService` -> `tickTechniqueActivity('gather')` -> `GatherStrategy.executeTick` -> `gather-tick.helpers.ts`；旧 `WorldRuntimeLootContainerService.tickGather` 仅保留兼容 facade | `player.gatherJob` + 容器 `activeSearch` | tick 推进、完成、采集经验、入包、续采/结束和 sleeping 已迁入 strategy helper；start/cancel 仍调用 loot service 窄 helper 注册/释放 activeSearch |
...
| 建造 | `startBuilding` 命令 -> `TechniqueActivityPipelineService.start('building')` -> `dispatchStartBuildingConstruction` | `WorldRuntimeCraftTickService` -> `tickTechniqueActivity('building')` -> `BuildingStrategy.executeTick` -> `building-tick.helpers.ts`；旧 `tickBuildingConstruction` 仅保留兼容 facade | `player.buildingJob` + 建筑 `activeBuilderPlayerId` | tick 推进、完工激活、建造经验、activeBuilder 清理和 building/active_job/profession dirty 已迁入 strategy helper；`MapInstanceRuntime.tickOnce()` 不再自动递减施工进度 |
| 阵法维护 / 补充灵力 | `startFormationMaintenance` -> `TechniqueActivityPipelineService.start('formation')`；`refillFormation` 是待命名澄清的一次性资源操作 | `TechniqueActivityPipelineService.tick('formation')` -> `FormationStrategy.resolve` -> `formation-maintenance-tick.helpers.ts`；旧 formation service 不再暴露 `resolveFormationMaintenanceTick` | `player.formationJob`，持续维护每息注入灵力 | 持续补充灵力必须是 `formation` job 并进入任务列表；一次性补给只能作为无进度资源操作，不能承载技艺经验、打断等待或任务进度 |
...
- 技艺经验：炼丹/炼器/强化在 pipeline tick helper 内算；采集在 `gather-tick.helpers.ts` 内算；挖矿在地块攻击链路内算；阵法维护由 `FormationStrategy.resolve` 产出统一 `expParams` 后经 pipeline 公共经验阶段结算；建造在 `building-tick.helpers.ts` 内算。
...
| 采集 | `StartGather` -> `GatherStrategy.validateStart/consumeResources/createJob` -> `WorldRuntimeLootContainerService.dispatchStartGather` 注册 activeSearch | `GatherStrategy.executeTick` -> `gather-tick.helpers.ts`；完成授予、经验、容器剩余、续采和 activeSearch 推进由 strategy helper 承载，旧 `tickGather` 只作兼容 facade | active job、inventory、profession、container_state；采集完成只改运行态背包并标记 dirty domain，禁止在 tick 内调用 durable grant / presence fence | 统一任务列表可见 running/sleeping/cancel；掉落/容器面板不再是唯一入口 |
...
| 建造 | `startBuilding` 命令 -> `BuildingStrategy` 标准 lifecycle -> `dispatchStartBuildingConstruction` 注册 activeBuilder 并创建 job | `BuildingStrategy.executeTick` -> `building-tick.helpers.ts`；建筑状态推进、完工激活、经验和 activeBuilder 清理由 strategy helper 承载，旧 `tickBuildingConstruction` 仅回调 helper | active job、profession、building 实例域；activeBuilder 释放会标记 building persistence dirty | 统一任务列表显示建造 job 和 sleeping；建筑面板只是专用操作区 |
| 阵法维护 / 持续注入 | `startFormationMaintenance` -> `TechniqueActivityPipelineService.start('formation')` -> `FormationStrategy.validateStart/queueStart/createJob` | `FormationStrategy.resolve` -> `formation-maintenance-tick.helpers.ts`；持续消耗玩家灵力、推进阵法维护，并由 pipeline 公共经验阶段结算阵法经验 | active job、profession、formation 实例状态；一次性 `refillFormation` 另走资源管理 | 统一任务列表显示维护 job；一次性补给不进入任务进度 |
...
- 进度递减路径：炼丹/炼器 `executeAlchemyLikeTick`、强化 `executeEnhancementTick`、采集 `gather-tick.helpers.ts`、建造 `building-tick.helpers.ts`、挖矿 `MiningStrategy.executeTick`、公共 pipeline 默认 tick / 阵法维护 resolve。
...
- 建造 `building-tick.helpers.ts` 每息递减建筑实例真实 `buildRemainingTicks`，并同步 `totalTicks/workTotalTicks/remainingTicks/workRemainingTicks`；这是 job 驱动的施工进度推进，不是打断等待。`MapInstanceRuntime.tickOnce()` 只清理失效 activeBuilder，不再自动完成施工。
- 已审计的打断来源入口：
...
- `world-runtime-building.service` / `building-tick.helpers.ts`：建造 start 和 completion 已改为 strategy result 返回结构化 `notice.craft.building.*`，再由统一 `flushCraftMutation` 分发；旧 direct facade 只保留结构化 notice fallback。建造放置失败错误、少量 direct facade 本地化错误仍是命令错误文本，不作为 result notice 完成项。
- `world-runtime-loot-container.service` / `gather-tick.helpers.ts`：采集 job 的 start/cancel/tick 生命周期消息已改为 strategy result 返回结构化 `notice.craft.gather.*`；普通地面/容器拿取仍属于 loot 系统，继续使用 `notice.loot.*`，不作为技艺 job notice 完成项。
- `world-runtime-formation.service`：阵法维护 job 的 start/queue/cancel/灵力不足中止已改为 strategy result 返回结构化 `notice.craft.formation.*`；布阵、开关、一次性补给、持续阵法注入、阵法灵力/灵石耗尽等阵法系统/资源管理 notice 已改为结构化 `notice.formation.*`。命令拒绝和参数错误仍是 exception 文本，不作为 result notice 完成项。
...
- [x] 区分 `start` 结果、`tick` 结果、`cancel` 结果，避免用 `CraftMutationResult` 承载所有情况；pipeline 已新增 `startLifecycle` / `tickLifecycle` / `cancelLifecycle` 返回明确 lifecycle 结果，旧 `start` / `tick` / `cancel` 仅作为现有 world tick / flush 调用面的兼容适配器。
...
- [x] 炼丹/炼器打断等待写入已从 `AlchemyStrategy` / `ForgingStrategy` 的 `executeInterrupt` 委托迁入公共 `TechniqueActivityPipelineService.interrupt`；旧 `interruptAlchemy` 仅保留为兼容 facade。
...
- [x] 炼丹/炼器 cancel 不再暴露 `AlchemyStrategy.executeCancel` / `ForgingStrategy.executeCancel` 完整委托，公共 `cancelLifecycle` 通过 `computeRefund` 物化退款，再统一清 active job。
...
- [x] 从 `startEnhancement` 拆出目标解析、资源校验、锁定空间写入、job 创建，并让 `EnhancementStrategy` start 走 pipeline 公共生命周期；旧 `startEnhancement` 仅保留为兼容 facade。
- [x] 从 `tickEnhancement` 拆出暂停推进、成功判定、保护物消耗、失败降级/归零、经验、完成结算和连续冲级 advance，放入 `enhancement-tick.helpers.ts`；旧 `tickEnhancement` 仅保留为兼容 facade。
- [x] 强化打断等待写入已从 `EnhancementStrategy.executeInterrupt` 委托迁入公共 `TechniqueActivityPipelineService.interrupt`；旧 `interruptEnhancement` 仅保留为兼容 facade，打断通知走结构化 `notice.craft.activity-interrupted-wait-generic`。
- [x] 强化 start、tick、advance、cancel、interrupt 通知改为结构化 `key/vars/pills`，不再在强化 pipeline 链路拼玩家可见中文。
- [x] 强化记录写入作为 strategy 生命周期中的显式 hook：start 走 `recordEnhancementStart`，单阶结算走 `recordEnhancementStepResult`，完成/停止/取消走 `completeEnhancementRecord`，不再隐藏在无语义的旧 service 内联块里。
- [x] 强化取消走公共 `cancelLifecycle` -> `computeRefund`，释放锁定装备、清 active job 和强化记录状态仍复用权威 `finishEnhancementJob`。
- [x] 强化 cancel 不再暴露 `EnhancementStrategy.executeCancel` 完整委托，公共 cancel lifecycle 负责统一清 active job、dirty 和 result 输出。
...
- [x] 阵法维护 tick/resolve 不再走旧 `WorldRuntimeFormationService.resolveFormationMaintenanceTick`；每息注入灵力、阵法池更新、实例 revision touch、延迟刷盘触发、活动记录和阵法经验参数改由 strategy helper 承载，旧 service 降级为阵法查询、预算读写、条件检查和持久化 helper。
- [x] 阵法维护 start/queue/cancel/灵力不足中止通知不再由 formation service 或 strategy helper 直接拼中文作为 job result；`FormationStrategy.buildStartMessages`、`FormationStrategy.queueStart`、`FormationStrategy.computeRefund` 和 `formation-maintenance-tick.helpers.ts` 返回结构化 `notice.craft.formation.*`，由统一 mutation flush 分发。布阵、开关、一次性补给仍是阵法系统/资源管理通知，不纳入本项。
...
- [x] 采集 start/cancel 不再走 `GatherStrategy.executeStart/executeCancel` 旧委托；启动校验、真实容器采集注册、job 创建、active_job dirty 和取消释放 activeSearch 改由 strategy 标准 lifecycle 插槽处理。
- [x] 采集 tick 不再走旧 `WorldRuntimeLootContainerService.tickGather` 真实规则；每息 activeSearch 推进、完成入包、采集经验、craft realm、续采/结束、容器刷新和 dirty domain 改由 `gather-tick.helpers.ts` 承载，旧 `tickGather` 降级为兼容 facade。
- [x] 建造 tick 不再走旧 `tickBuildingConstruction` 真实规则；每息递减 `buildRemainingTicks`、同步 job 进度、结算建造经验、完工激活建筑 topology/visual、清理 activeBuilder、刷新 context action 和 dirty domain 改由 `building-tick.helpers.ts` 承载，旧 `tickBuildingConstruction` 降级为兼容 facade。
- [x] 建造 start/completion 通知不再由 building service 或 tick helper 直接拼中文入队；`BuildingStrategy.buildStartMessages` 和 `building-tick.helpers.ts` 返回结构化 `notice.craft.building.*`，由统一 mutation flush 分发。
- [x] 采集 start/cancel/tick 通知不再由 loot container service 或 tick helper 直接拼中文作为 job result；`GatherStrategy.buildStartMessages`、`GatherStrategy.computeRefund` 和 `gather-tick.helpers.ts` 返回结构化 `notice.craft.gather.*`，由统一 mutation flush 分发。普通地面/容器拿取仍是 loot 系统通知，不纳入本项。
...
- [x] `MapInstanceRuntime.tickOnce()` 不再自动推进 activeBuilder 施工；实例 tick 只清理离线、离开实例或离开范围的失效 activeBuilder，建造进度只能由 `BuildingStrategy` / `tickBuildingConstruction` facade 后的统一 job tick 推进。
...
- [ ] tick 内无数据库 IO、无配置解析、无高频 JSON 签名比较。（采集完成 durable grant 缺口已处理且采集 tick 规则已迁入 strategy helper；阵法维护 tick 派生的延迟刷盘不再懒初始化 DB/schema；仍需继续审计其他旧 runtime 委托路径，不能据此标记整个 Phase 6 完成。）
...
- [ ] flush ledger / active job version 只在统一路径 bump。（公共 pipeline dirty 路径已经在 `active_job` dirty 时统一 bump jobVersion，阵法维护 tick helper 的手动 bump 已移除；但 `CraftPanelRuntimeService.finalizeMutation` 和 `MiningStrategy.executeTick` 仍有 active job version bump 残留，不能据此勾选完成。）
...
- 2026-05-27：炼丹/炼器 pipeline 通知改为结构化 `key/vars/pills`：启动、入队、打断等待、取消、批次成功/失败、完成、队列续跑/跳过都不再在该链路拼玩家可见中文；新增 `notice.craft.alchemy.*`、`notice.craft.queue.*`、`notice.craft.activity-interrupted-wait` i18n 源和生成产物，`world-runtime-alchemy-smoke` 改为断言 structured key。`pnpm --filter @mud/server compile`、`pnpm --filter @mud/client exec tsc --noEmit --pretty false`、`node packages/server/dist/tools/world-runtime-alchemy-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/technique-activity-cancel-ref-smoke.js`、`pnpm audit:protocol`、`pnpm verify:client`、`pnpm verify:quick` 通过；`verify:quick` 中 session reaper 的 `simulated_flush_failure` 是用例内故障注入且最终通过。其他旧 runtime service 文本 notice 仍待后续迁移。
- 2026-05-27：炼丹/炼器 cancel 从 `AlchemyStrategy` / `ForgingStrategy` 委托旧 `cancelAlchemy/cancelForging` 改为 `executeAlchemyLikeCancel` strategy helper；取消时计算未开始批次退料、退灵石、退料入包/掉地、清 active job 和 active_job dirty，退料入包/掉地复用公共 `applyTechniqueActivityResolveInventory`。`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-alchemy-smoke.js`、`node packages/server/dist/tools/technique-activity-cancel-ref-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`pnpm verify:quick` 通过；`verify:quick` 中 session reaper 的 `simulated_flush_failure` 是用例内故障注入且最终通过。
- 2026-05-27：炼丹/炼器 interrupt 曾从 `AlchemyStrategy` / `ForgingStrategy` 委托旧 `interruptAlchemy` 改为 strategy helper；后续已继续迁入公共 `TechniqueActivityPipelineService.interrupt`，当前不再保留 `executeAlchemyLikeInterrupt` helper 或 strategy `executeInterrupt` 委托。
- 2026-05-27：强化 cancel 从 `EnhancementStrategy` 委托旧 `cancelEnhancement` 改为 `executeEnhancementCancel` strategy helper；取消仍调用权威 `finishEnhancementJob` 释放 `inventory.lockedItems` 工件、回写背包/装备、清 active job 并记录 `cancelled` 状态，返回结构化 `notice.craft.enhancement.cancelled`。`world-runtime-enhancement-smoke` 的取消用例改为走 `cancelTechniqueActivity(player, 'enhancement')`。`pnpm --filter @mud/server compile`、`pnpm --filter @mud/client exec tsc --noEmit --pretty false`、`node packages/server/dist/tools/world-runtime-enhancement-smoke.js`、`node packages/server/dist/tools/enhancement-panel-payload-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`node packages/server/dist/tools/technique-activity-cancel-ref-smoke.js`、`pnpm verify:quick`、`pnpm verify:client` 通过；`enhancement-panel-payload-smoke` 的 legacy itemInstanceId WARN 来自夹具水合，`verify:quick` 中 session reaper 的 `simulated_flush_failure` 是用例内故障注入且最终通过。
- 2026-05-27：强化 start 生命周期从 `CraftPanelRuntimeService.startEnhancement` 整段实现拆为 `validateEnhancementStart`、`queueEnhancementStart`、`consumeEnhancementStartResources`、`createEnhancementStartJob` 和 `finalizeEnhancementStart`；`EnhancementStrategy` 移除 `executeStart`，由 `TechniqueActivityPipelineService.start` 依次调用 strategy 校验、排队、锁定/扣料和创建 job。旧 `startEnhancement` 只保留为兼容 facade 回到 `startTechniqueActivity('enhancement')`，队列中的强化项也因此经同一 pipeline start 启动；已有其他技艺活动时仍只写 `techniqueActivityQueue`，不提前锁装备、不扣材料/灵石。`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-enhancement-smoke.js`、`node packages/server/dist/tools/enhancement-panel-payload-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`node packages/server/dist/tools/world-runtime-craft-smoke.js` 通过；`enhancement-panel-payload-smoke` 的 legacy itemInstanceId WARN 来自夹具水合。
- 2026-05-27：强化 tick 生命周期从 `CraftPanelRuntimeService.tickEnhancement` 整段实现拆入 `packages/server/src/runtime/craft/pipeline/strategies/enhancement-tick.helpers.ts`，`EnhancementStrategy.executeTick` 直接推进暂停恢复、实际工作量、成功/失败判定、保护物消耗、灵石扣费、技艺经验、连续冲级、完成结算和队列下一项；旧 `tickEnhancement` 只保留兼容 facade，回调 `tickTechniqueActivity('enhancement')`。`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-enhancement-smoke.js`、`node packages/server/dist/tools/enhancement-panel-payload-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`node packages/server/dist/tools/world-runtime-craft-smoke.js` 通过；`enhancement-panel-payload-smoke` 的 legacy itemInstanceId WARN 来自夹具水合。强化记录已拆为显式 hook，但公共 `recordDelta` 物化仍待后续迁移。
- 2026-05-27：强化 interrupt 生命周期曾从 `CraftPanelRuntimeService.interruptEnhancement` 整段实现拆入 strategy helper；后续已继续迁入公共 `TechniqueActivityPipelineService.interrupt`，当前不再保留 `enhancement-interrupt.helpers.ts` 或 `EnhancementStrategy.executeInterrupt` 委托。
...
- 2026-05-27：阵法维护 start/cancel 从旧委托迁入标准 pipeline lifecycle：`FormationStrategy` 不再实现 `executeStart` / `executeCancel`，而是通过 `validateStart`、`queueStart`、`createJob`、`buildStartMessages` 和 `computeRefund` 承载启动校验、互斥入队、job 创建、启动通知和取消通知。`world-runtime-craft-smoke` 的 sleeping 阵法队列恢复 proof 改为断言旧 `startFormationMaintenance` 被调用即失败，并证明恢复会通过 pipeline start 创建 `formationJob`、标记 `active_job` dirty；`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/world-runtime-formation-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`node packages/server/dist/tools/technique-activity-cancel-ref-smoke.js` 通过。该切片当时只迁出 start/cancel，tick/resolve 后续见下条迁移记录。
- 2026-05-27：阵法维护旧 start/cancel service 入口删除：`WorldRuntimeFormationService.startFormationMaintenance`、`cancelFormationMaintenance` 和旧 service 内 `enqueueFormationMaintenance` helper 已移除，避免阵法维护出现 service 私有启动/入队/取消路径；启动、互斥入队、取消只剩 `FormationStrategy` + `TechniqueActivityPipelineService` lifecycle。`world-runtime-craft-smoke` 增加结构性 proof，断言 `WorldRuntimeFormationService.prototype` 不再暴露旧 start/cancel，并保留 sleeping 阵法队列通过 pipeline start 恢复的行为 proof。`git diff --check`、`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/world-runtime-formation-smoke.js` 通过。该切片当时还未迁出 tick/resolve，后续见下条迁移记录。
- 2026-05-27：阵法维护 tick/resolve 从旧 formation service 迁入 strategy helper：`FormationStrategy.resolve` 直接调用 `formation-maintenance-tick.helpers.ts`，旧 `WorldRuntimeFormationService.resolveFormationMaintenanceTick` 已删除；每息维护速率、玩家灵力消耗、阵法灵力池更新、阵法 active 状态、实例 revision touch、延迟刷盘触发、活动记录、job 1 息重置和阵法经验参数都由 strategy helper 产出，formation service 只保留阵法查询、预算读写、条件检查和持久化 helper。`world-runtime-craft-smoke` 增加结构性 proof，断言旧 service 不再暴露 `resolveFormationMaintenanceTick`，并用 `TechniqueActivityPipelineService + FormationStrategy` 在旧 service resolve 缺失时完成维护 tick；`git diff --check`、`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/world-runtime-formation-smoke.js` 通过。采集/建造 tick 仍委托旧 runtime service，阵法 service 也仍承载阵法状态和持久化 helper，所以不能据此勾选“旧 service 不再承载技艺玩法规则”。
...
- 2026-05-27：采集 start/cancel 从旧委托迁入标准 pipeline lifecycle：`GatherStrategy` 不再实现 `executeStart` / `executeCancel`，而是通过 `validateStart`、`consumeResources`、`createJob`、`startDirtyDomains`、`computeRefund` 和条件型 `onConditionFailed` 承载启动校验、调用真实采集 runtime 注册 activeSearch、读取 created job、标记 active_job、取消时释放 activeSearch。`world-runtime-craft-smoke` 增加采集 lifecycle proof，覆盖 sleeping 恢复通过 pipeline start 创建 `gatherJob`、取消通过公共 cancel 释放 activeSearch 并标记 active_job dirty；`world-runtime-loot-container-smoke` 继续证明采集完成 tick 不调用 durable grant / presence fence。`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/world-runtime-loot-container-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`node packages/server/dist/tools/technique-activity-cancel-ref-smoke.js` 通过。该切片当时还未迁出采集 tick，后续见采集 tick helper 记录。
- 2026-05-27：采集 tick 真实规则从 `WorldRuntimeLootContainerService.tickGather` 迁入 `GatherStrategy` helper：`GatherStrategy.executeTick` 直接调用 `gather-tick.helpers.ts`，每息推进 activeSearch、完成时只更新运行态背包并标记 dirty、结算采集技艺经验/craft realm、刷新 quest、处理续采/采尽/休眠；旧 `tickGather` 保留为兼容 facade，同样回调该 helper。`world-runtime-craft-smoke` 增加 proof，fake loot service 不提供 `tickGather` 也能通过 `TechniqueActivityPipelineService + GatherStrategy` 完成一次采集结算；`world-runtime-loot-container-smoke` 继续覆盖旧 facade 行为不回退。`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/world-runtime-loot-container-smoke.js` 通过。建造 tick 仍委托 `tickBuildingConstruction`，所以仍不能勾选“旧 service 不再承载技艺玩法规则”总项。
- 2026-05-27：建造 tick 真实规则从旧 `tickBuildingConstruction` 主体迁入 `BuildingStrategy` helper：`BuildingStrategy.executeTick` 直接调用 `building-tick.helpers.ts`，每息递减建筑 `buildRemainingTicks`、同步 job 进度、结算建造经验、完工时激活建筑 topology/visual、清理 activeBuilder、标记 building/active_job/profession dirty 并刷新 context action；旧 `tickBuildingConstruction` 只保留兼容 facade 回调 helper。`MapInstanceRuntime.advanceBuildingConstruction()` 不再递减施工进度或自动完工，只清理离线、离开实例或离开范围的失效 activeBuilder。`world-runtime-craft-smoke` 增加 proof，fake deps 不提供 `tickBuildingConstruction` 也能通过 `TechniqueActivityPipelineService + BuildingStrategy` 完成建造；`world-runtime-building-room-fengshui-smoke` 改为断言实例 tick 不自动施工，必须经 job tick 完工激活。`git diff --check`、`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/world-runtime-building-room-fengshui-smoke.js` 通过。building service 仍承载建造放置/拆除/查询/通知等 helper，所以不能据此勾选“旧 service 不再承载技艺玩法规则”总项。
- 2026-05-27：建造 start/completion 通知改为结构化 result：`BuildingStrategy.buildStartMessages` 返回 `notice.craft.building.start`，`building-tick.helpers.ts` 完工返回 `notice.craft.building.completed`，目标缺失/不可施工/转入 sleeping 返回 `notice.craft.building.*` 结构化 key；这些消息随 pipeline result 交给 `WorldRuntimeCraftMutationService.flushCraftMutation` 分发。旧 `handleStartBuildingConstruction` 不再直接入队“开始建造”文本，`interruptBuildingConstruction` / `notifyBuildingConstructionCompletion` 仅作为 direct facade 保留结构化 fallback。新增 i18n 文案 `notice.craft.building.*` 并生成客户端产物；`world-runtime-craft-smoke` 断言建造 start/completion 返回 key/vars。`git diff --check`、`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/client exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/world-runtime-building-room-fengshui-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`pnpm verify:quick` 通过。该 proof 只覆盖建造通知，不等同于采集/阵法/命令阻塞等所有技艺文本 notice 已迁完。
- 2026-05-27：采集 start/cancel/tick 通知改为结构化 result：`GatherStrategy.buildStartMessages` 返回 `notice.craft.gather.start`，`GatherStrategy.computeRefund` 返回 `notice.craft.gather.cancelled`，`gather-tick.helpers.ts` 在目标缺失、离开范围、目标占用转 sleeping、采尽、空容器和获得物品时返回 `notice.craft.gather.*` 结构化 key；旧 `WorldRuntimeLootContainerService.dispatchStartGather` / `dispatchCancelGather` / `interruptGather` 只保留结构化 fallback。新增 i18n 文案 `notice.craft.gather.*` 并生成客户端产物；`world-runtime-loot-container-smoke` 断言采集 start/completion 返回 key/vars。普通地面/容器拿取仍使用 `notice.loot.*`，不属于技艺 job notice。`git diff --check`、`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/client exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-loot-container-smoke.js`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`pnpm verify:quick` 通过；`verify:quick` 中 session reaper 的 `simulated_flush_failure` 是用例内故障注入且最终通过。
- 2026-05-27：阵法维护 start/queue/cancel/灵力不足中止通知改为结构化 result：`FormationStrategy.queueStart` 返回 `notice.craft.formation.already-maintaining` / `notice.craft.formation.queued`，`FormationStrategy.buildStartMessages` 返回 `notice.craft.formation.start`，`FormationStrategy.computeRefund` 返回 `notice.craft.formation.stopped`，`formation-maintenance-tick.helpers.ts` 在玩家灵力不足时返回 `notice.craft.formation.qi-insufficient`；新增 i18n 文案 `notice.craft.formation.*` 并生成客户端产物。`world-runtime-craft-smoke` 断言阵法维护 queue restart、cancel 和灵力不足中止返回 key/vars，`world-runtime-formation-smoke` 断言阵法维护 start 返回 key/vars。布阵、开关、一次性补给仍是阵法系统/资源管理通知，不属于持续维护 job notice。`git diff --check`、`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/client exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`node packages/server/dist/tools/world-runtime-formation-smoke.js`、`pnpm verify:quick` 通过；`world-runtime-formation-smoke` 中“阵法持久化池未就绪”WARN 来自既有 timer proof，`verify:quick` 中 session reaper 的 `simulated_flush_failure` 是用例内故障注入且最终通过。
- 2026-05-27：阵法系统/资源管理 notice 改为结构化载荷：`WorldRuntimeFormationService` 的布阵、开关、一次性补给、持续阵法注入、灵石耗尽损毁、灵力不足关闭、阵眼灵力耗尽关闭不再直接 enqueue 纯文本 notice，而是统一经 `buildStructuredNotice` 发送 `notice.formation.*` key/vars/pills。新增 i18n 文案并生成客户端产物；`world-runtime-formation-smoke` 断言布阵、补给、开关、阵眼灵力耗尽和持续阵法注入返回 structured key/vars。`git diff --check`、`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/client exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-formation-smoke.js`、`pnpm verify:quick` 通过；命令拒绝和参数错误仍是 exception 文本，不作为 result notice 完成项，`verify:quick` 中 session reaper 的 `simulated_flush_failure` 是用例内故障注入且最终通过。
- 2026-05-27：炼丹/炼器 cancel 从 `executeCancel` 完整委托迁入标准 pipeline lifecycle：`AlchemyStrategy` / `ForgingStrategy` 不再实现 `executeCancel`，取消退款由 `computeAlchemyLikeCancelRefund` 在 `computeRefund` 阶段物化，负责退未开始批次材料、退灵石、入包或掉地并标记背包相关变更；公共 `cancelLifecycle` 随后统一清 active job、标记 active_job dirty 并返回 cancel lifecycle 结果。`world-runtime-alchemy-smoke` 增加结构性 proof，断言炼丹/炼器 strategy 不再暴露 `executeCancel` 且具备 `computeRefund`；同时保留炼丹/炼器取消、掉地、队列和打断行为 proof。`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-alchemy-smoke.js`、`node packages/server/dist/tools/technique-activity-cancel-ref-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js` 通过。炼丹/炼器 tick 仍通过 strategy helper 走 `executeTick`，所以不能据此勾选“所有 runtime kind 的 start/cancel/interrupt/tick 都通过公共 lifecycle”。
...
- 2026-05-27：统一任务列表取消 proof 补齐：`technique-activity-cancel-ref-smoke` 先从 `buildTechniqueActivityTaskListView` 取得 cancelRef，再调用 `dispatchCancelTechniqueActivityByRef`，覆盖 running、interrupt_wait、queued、sleeping 四类任务；queued/sleeping 会删除统一队列项并 flush 面板，running/interrupt_wait 会按 jobRunId 校验后派发 active job cancel，旧制造队列兼容项仍可取消，stale jobRunId 不会误取消新任务。`git diff --check`、`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/technique-activity-cancel-ref-smoke.js`、`node packages/server/dist/tools/technique-activity-task-view-smoke.js`、`pnpm verify:quick` 通过；`verify:quick` 中 session reaper 的 `simulated_flush_failure` 是用例内故障注入且最终通过。
- 2026-05-27：打断等待重复刷新和恢复 proof 补齐：`world-runtime-craft-smoke` 增加 `testRepeatedInterruptAndResumeKeepWorkProgressSeparate`，同一 active job 首次打断会写独立等待，重复打断不会叠加等待、不改 reason、不污染 `totalTicks` / `remainingTicks` / `workTotalTicks` / `workRemainingTicks`；随后通过 pipeline tick 逐息推进等待条，直到恢复原 phase，全程实际工作进度不变。`git diff --check`、`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/world-runtime-craft-smoke.js`、`pnpm verify:quick` 通过；`verify:quick` 中 session reaper 的 `simulated_flush_failure` 是用例内故障注入且最终通过。
- 2026-05-27：炼丹、炼器、强化的 `executeInterrupt` 完整委托口移除，公共 `TechniqueActivityPipelineService.interrupt` 统一处理非条件型技艺的暂停等待、结构化通用打断通知和 `active_job` dirty；`markPipelineDirty` 在 `active_job` dirty 时统一 bump active job version，阵法维护 tick helper 不再手动递增 `jobVersion`。`craft-persistence-dirty-domain-smoke` 补齐统一队列分域直写 mock，并用稳定 `itemInstanceId` 引用强化目标，继续证明 active job version 随 craft 变更单调前进。`pnpm --filter @mud/server exec tsc --noEmit --pretty false`、`pnpm --filter @mud/server compile`、`node packages/server/dist/tools/craft-persistence-dirty-domain-smoke.js`、`node packages/server/dist/tools/world-runtime-alchemy-smoke.js`、`node packages/server/dist/tools/world-runtime-enhancement-smoke.js`、`node packages/server/dist/tools/world-runtime-craft-smoke.js` 通过。`CraftPanelRuntimeService.finalizeMutation` 和 `MiningStrategy.executeTick` 仍有 active job version bump 残留，所以 Phase 7 version 总项仍未完成。
