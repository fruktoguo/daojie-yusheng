...
| `SERVER_FORMATION_MAINTENANCE_CHECKPOINT_INTERVAL_MS` | `10000` | 阵法维护跨域检查点合并窗口，范围 `1000-60000ms` |
...
GM 配置写入与启动期数据库回载使用同一注册表校验。历史表中若存在非整数、越界或类型错误的值，启动时会记录告警并跳过该行，由现有环境变量或代码内生产默认值接管，不把脏配置注入运行时。

| 环境变量 | 默认值 |
|----------|--------|
| `SERVER_ENCODING_WORKER_COUNT` | `6` |
| `SERVER_INSTANCE_WORKER_COUNT` | `6` |
| `SERVER_PERSISTENCE_WORKER_COUNT` | `2` |
