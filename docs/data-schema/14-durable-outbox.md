...
- Dispatcher 辅助表初始化失败时禁用事件消费但不阻断核心服务启动；已提交事件保持待处理，修复数据库后需重启恢复消费
...
- 热表中超过 30 天的记录分批迁入 archive 表
- archive 按事件 `created_at` 计算总保留期并分批清理：普通资产审计默认保留 365 天，`combat` 战斗审计默认保留 90 天
- 每轮迁移与清理均有 batch limit，删除只释放 PostgreSQL 可复用空间；关系文件物理缩容需另行安排受控维护窗口
